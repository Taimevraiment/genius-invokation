...
import {
    ActionData, CalcAtkRes, Card, Cmds, Countdown, DamageVO, Hero, LogType, MinuDiceSkill, Player, Preview, ServerData, Skill, Status, StatusTask,
    Summon, Support, Trigger
} from '../../typing';
...
export default class GeniusInvokationRoom {
    ...
    private _randomInt(max: number = 1) {
        ...
        const randomInt = Math.floor(this._random % 1e6 / 1e6 * (max + 1));
        return randomInt;
    }
    /**
     * 获取数组中随机一项
    ...
    emit(flag: string, pidx: number, options: {
        ...
        isQuickAction?: boolean, canAction?: boolean, isChange?: boolean, isActionInfo?: boolean, slotSelect?: number[],
        heroSelect?: number[], statusSelect?: number[], summonSelect?: number[], supportSelect?: number[],
    } = {}) {
        const { socket, tip = '', actionInfo = '', damageVO = -1, notPreview = false, isActionInfo = false, canAction = true,
            ...
            isQuickAction = false, isChange = false } = options;
        this.players.forEach(p => {
            p.handCards.forEach((c, ci) => c.cidx = ci);
            p.heros.forEach(h => {
                if (!notUpdate) assgin(h.heroStatus, this._updateStatus(p.pidx, [], h.heroStatus, p, true, h.hidx));
                this._calcSkillChange(p.pidx, h.hidx);
            });
            if (!notUpdate) assgin(p.combatStatus, this._updateStatus(p.pidx, [], p.combatStatus, p, true, p.hidx));
            assgin(p.summons, this._updateSummon(p.pidx, [], p.summons, p, true));
    ...
    getActionDev(actionData: {
        ...
        disCardCnt: number, smnIds: number[], sptIds: number[], flag: string
    }) {
    ...
    private async _switchHero(pidx: number, hidx: number, flag: string,
        ...
        if (isDieSwitch) { // 被击倒后选择出战角色
            ...
            const { isQuickAction: slotiqa } = this._detectSlot(pidx ^ 1, 'kill', { isQuickAction });
            ...
            player.status = isOppoActioning ? PLAYER_STATUS.WAITING : PLAYER_STATUS.PLAYING;
        ...
        this.taskQueue.addTask('switchHero', [[async () => {
            ...
            this._detectStatus(pidx, STATUS_TYPE.Attack, 'change-from', { hidxs: [ohidx], fhidx: ohidx, isOnlyHeroStatus: true, isQuickAction: isCdt(isQuickAction, 2) });
            this._detectStatus(pidx, STATUS_TYPE.Attack, 'change-to', { hidxs: [hidx], isOnlyHeroStatus: true, isQuickAction: isCdt(isQuickAction, 2) });
            this._detectStatus(pidx, STATUS_TYPE.Attack, 'change-from', { isOnlyCombatStatus: true, isQuickAction: isCdt(isQuickAction, 2) });
            this.emit(flag, pidx, { isQuickAction: isQuickAction && this.taskQueue.isTaskEmpty(), isActionInfo: true });
        }, 1e3]], true);
    ...
    private _useSkill(pidx: number, skid: number | SkillType, options: {
        ...
        const { isPreview = false, withCard, isSwitch = -1, isReadySkill = false, otriggers = [],
            willDamages, dmgElement, isSummon = -1, atkedIdx, players: cplayers = this.players } = options;
        ...
        const calcAtk = (oplayers: Player[], res: CalcAtkRes, type: string, stsId: number, skid = -1, isSelf = 0) => {
            ...
            if (res.heal) {
                ...
                if (type.includes('Status')) {
                    ...
                    const { willHeals: whl } = this._doCmds(cpidx, res.cmds, {
                        hidxs: [atkHidx],
                        players: oplayers,
                        isAction: !isQuickAction,
                        supportCnt,
                ...
                mergeWillHeals(bWillHeal, willHeals, oplayers);
            }
            ...
            const { willDamages: willDamage3, willAttachs: willAttachs3,
            ...
            const { isSwitch: csw = -1, isSwitchOppo: cswo = -1 } = this._doCmds(cpidx, elrcmds3[0], {
                players: players3, isExec: false, isAction: !isQuickAction, willSwitch, supportCnt,
            });
            if (type == 'die') return true;
            let obj: Summon | Status | undefined;
            if (type.includes('Status')) {
                if (type == 'combatStatus') obj = oplayers[cpidx].combatStatus.find(sts3 => sts3.id == stsId);
                else if (type == 'heroStatus') obj = oplayers[cpidx].heros[isSelf ? cahidx : cehidx].heroStatus.find(sts3 => sts3.id == stsId);
                res.exec?.(obj, { heros: oplayers[cpidx].heros, eheros: oplayers[cpidx ^ 1].heros });
            }
            players3.forEach(p => {
                p.heros.forEach(h => {
                    h.hp -= willDamage3[p.pidx * players3[0].heros.length + h.hidx].reduce((a, b) => a + Math.max(0, b), 0);
                });
            });
            ...
            // const oehidx = cehidx;
            if (isSelf) {
                cahidx = csw > -1 ? csw : cahidx;
                cehidx = cswo > -1 ? cswo : cehidx;
            } else {
                cahidx = cswo > -1 ? cswo : cahidx;
                cehidx = csw > -1 ? csw : cehidx;
            }
            if (isSwitchOppo(elrcmds3[0])) atriggers3[cahidx].push('change-from', 'change-to');
            doPreviewStatus(oplayers, oplayers[pidx].heros[cahidx].heroStatus, cahidx, STATUS_GROUP.heroStatus, atriggers3[cahidx], isExec, 1);
            doPreviewStatus(oplayers, oplayers[pidx].combatStatus, cahidx, STATUS_GROUP.combatStatus, atriggers3[hidx], isExec, 1);
            if (isSwitchOppo(elrcmds3[0])) etriggers3[cehidx].push('change-from', 'change-to');
            doPreviewStatus(oplayers, oplayers[epidx].heros[cehidx].heroStatus, cehidx, STATUS_GROUP.heroStatus, etriggers3[cehidx], isExec);
            doPreviewStatus(oplayers, oplayers[epidx].combatStatus, cehidx, STATUS_GROUP.combatStatus, etriggers3[odmgedHidx], isExec);
            return false;
        }
        const doPreviewStatus = (oplayers: Player[], ostatus: Status[], hi: number, group: StatusGroup, trgs: Trigger[], isExec: boolean, isSelf = 0) => {
            const status = clone(ostatus);
            const cpidx = pidx ^ +!isSelf;
            const chidx = isSelf ? cahidx : cehidx;
            const atkStatus: [StatusTask, boolean?][] = [];
            preview_end: for (const sts of status) {
                ...
                for (const state of trgs) {
                    ...
                    if (sts.hasType(STATUS_TYPE.Attack) && (stsres.damage || stsres.pdmg || stsres.heal)) {
                        if (isExec && !state.includes('change') && !state.includes('get')) {
                            atkStatus.push([{
                                id: sts.id,
                                name: sts.name,
                        ...
                        const willKill = isSelf ? dmg[pidx * ahlen + cehidx] >= oplayers[cpidx ^ 1].heros[cehidx].hp : dmg[(cpidx ^ 1) * ehlen + cahidx] >= oplayers[cpidx].heros[cahidx].hp;
                        if (stsres.notPreview || calcAtk(oplayers, stsres, willKill ? 'die' : (['hero', 'combat'][group] + 'Status'), sts.id, skid, isSelf)) break preview_end;
                    }
                }
            }
            return { atkStatus }
        ...
        if (isSwitch > -1 && !isExec) { // 切换角色后的协同攻击预览
            doPreviewStatus(players, players[pidx].heros[ahidx()].heroStatus, ahidx(), STATUS_GROUP.heroStatus, ['change-from'], false, 1);
            doPreviewStatus(players, players[pidx].heros[isSwitch].heroStatus, isSwitch, STATUS_GROUP.heroStatus, ['change-to'], false, 1);
            doPreviewStatus(players, players[pidx].combatStatus, isSwitch, STATUS_GROUP.combatStatus, ['change-from', 'change-to'], false, 1);
        }
        ...
        this._doCmds(pidx, stsprecmds, { players, ahidx: hidx, ehidx: dmgedHidx, isAction: !isQuickAction });
        if (skillres.summonPre) assgin(player().summons, this._updateSummon(pidx, this._getSummonById(skillres.summonPre), aSummons(), player()));
        if (skillres.heal) {
            const { willHeals } = this._doCmds(pidx, [{ cmd: 'heal', cnt: skillres.heal, hidxs: skillres.hidxs ?? [hidx] }], {
                players,
                isExec,
                isAction: !isQuickAction,
                supportCnt,
        ...
        if (dmgEl && oWillDamages) {
            ...
            skillcmds.push(...elrcmds1[0], ...(skillres.cmds ?? []));
            ...
            this._doCmds(pidx, stsaftercmds, { players, ahidx: hidx, ehidx: dmgedHidx, isAction: !isQuickAction });
            if (skillres.summon) assgin(aSummons(), this._updateSummon(pidx, this._getSummonById(skillres.summon), aSummons(), player()));
            if (skillres.isAttach) {
                const { elTips: elTips2 = [], willAttachs: willAttachs2 = [] } = this._doCmds(pidx, [{ cmd: 'attach', hidxs: [hidx] }], { players, isExec, isAction: !isQuickAction });
        ...
        bPlayers.forEach(p => {
            p.heros.forEach(h => {
                h.hp -= aWillDamages[p.pidx * bPlayers[0].heros.length + h.hidx].reduce((a, b) => a + Math.max(0, b), 0);
            });
        });
        const stscmds: Cmds[] = [
        ...
        this._doCmds(pidx, stscmds, { players, ahidx: hidx, ehidx: dmgedHidx, isExec, isAction: !isQuickAction });
        if (skill && isExec) {
            players[pidx].isFallAtk = false;
            players[pidx].canAction = false;
            this.taskQueue.addTask(`useSkill-${skill.name}`, [[() => {
                assgin(this.players, players);
                ...
                this._doDamage(pidx, {
                    ...
                    elTips: aElTips,
                    willHeals: aWillHeal,
                }, { isSwitch, cmds: energyCmds });
            }, 2250]], true);
        }
        const { willHeals: skillheal, isSwitch: swc = -1, isSwitchOppo: swco = -1, tasks = [] } = this._doCmds(pidx, isCdt(!isExec || !!skill, skillcmds), {
            players: bPlayers,
            isExec,
        ...
        cahidx = isSwitchSelf(skillcmds) && swc > -1 ? swc : skid == -1 && dtriggers.includes('change-to') ? isSwitch : ahidx();
        ...
        for (const smn of bPlayers[pidx].summons) {
            ...
            const strigger = [...dtriggers];
            if (skill) strigger.push(`after-skilltype${skill.type}`, 'after-skill')
            strigger.forEach(trg => {
                const { smnres } = this._detectSummon(pidx, trg, { csummon: [smn], heros: bPlayers[pidx].heros, hcard: withCard, isExec });
                if (smnres) calcAtk(bPlayers, smnres, 'summon', smn.id, skid, 1);
            });
        }
        if (isSwitchOppo(skillcmds)) {
            doPreviewStatus(bPlayers, bPlayers[epidx].heros[odmgedHidx].heroStatus, odmgedHidx, STATUS_GROUP.heroStatus, ['change-from', 'change-to'], isExec);
            doPreviewStatus(bPlayers, bPlayers[epidx].combatStatus, odmgedHidx, STATUS_GROUP.combatStatus, ['change-from', 'change-to'], isExec);
        }
        if (isSwitchSelf(skillcmds)) {
            const triggers: Trigger[] = skid == -1 ? dtriggers : ['change-from', 'change-to'];
            doPreviewStatus(bPlayers, bPlayers[pidx].heros[ahidx()].heroStatus, ahidx(), STATUS_GROUP.heroStatus, triggers, isExec, 1);
            doPreviewStatus(bPlayers, bPlayers[pidx].combatStatus, ahidx(), STATUS_GROUP.combatStatus, triggers, isExec, 1);
        }
        ...
        return {
            ...
            players, summonCnt, supportCnt, willSummons, willSwitch, willDamages: aWillDamages, skillcmds,
        }
    }
    /**
     * 计算伤害
    ...
    private _calcDamage(
        ...
        options: {
            ...
            elrcmds?: Cmds[][], atriggers?: Trigger[][], etriggers?: Trigger[][], usedDice?: number, dmgElements?: DamageType[], willSwitch?: boolean[][],
        ...
        const { isAttach = false, isSummon = -1, isSwirl = false, isExec = true, multiDmg = 0, skid = -1, isAtkSelf = 0,
            ...
            discards = [], withCard, isChargedAtk = false, isFallAtk = false, supportCnt = players.map(() => new Array(MAX_SUPPORT_COUNT).fill(0)),
            willSwitch = players.map(p => p.heros.map(() => false)),
        } = options;
            ...
            if ( // 没有元素反应(不含冰草共存)
                ...
                if (dmgElement == ELEMENT_TYPE.Anemo && elTypes.includes(attachElement)) { // 扩散
                    ...
                    if (hasEls(ELEMENT_TYPE.Cryo, ELEMENT_TYPE.Dendro)) { // 冰草共存
                        ...
                        assgin(efhero.heroStatus, this._updateStatus(pidx, [this.newStatus(106)], efhero.heroStatus, res.players[dmgedPidx]));
                        ...
                        if (efhero.isFront) res.elrcmds[0].push({ cmd: 'switch-after', isOppo: !isAtkSelf });
        ...
        if (!isAtkSelf) res.atriggers.forEach((trgs, hi) => detectStatus(trgs, hi, true));
        ...
        res.players[pidx].isFallAtk ||= res.isFallAtk;
        if (!isSwirl && isExec) {
            if (res.willDamages.some(d => Math.max(0, d[0]) + d[1] > 0)) {
                ...
                if (willKilledHidxs.length > 0) {
                    ...
                    if (slotDestroyHidxs.length > 0) {
                        this._detectStatus(dmgedPidx, STATUS_TYPE.Usage, 'slot-destroy', { hidxs: slotDestroyHidxs, players: res.players, isOnlyCombatStatus: true, isExec });
                    }
                }
            }
        }
     ...
     */
    private _checkCard(pidx: number, players: Player[], cardIdx: number, options: { isReconcile?: boolean, heroIdx?: number } = {}): {
        isValid: boolean,
        diceSelect: boolean[],
        supportCnt?: number[][],
        skillId?: number,
        ...
        const { isReconcile = false, heroIdx } = options;
        const player = players[pidx];
        ...
        const { isSwitch: switchIdx, attackPreview } = this._doCmds(pidx, cardres.cmds, { withCard: currCard, isExec: false });
    ...
    private async _useCard(pidx: number, cardIdx: number, diceSelect: boolean[], socket: Socket,
        ...
        await this._execTask();
        this._detectStatus(pidx, STATUS_TYPE.Usage, 'card', { card: currCard, hidxs: getBackHidxs(player.heros), isOnlyHeroStatus: true, minusDiceCard });
        await this._execTask();
        this._detectSkill(pidx, 'card');
        await this._execTask();
        this._detectSupport(pidx, 'card', { hcard: currCard, minusDiceCard: stsmdc, isQuickAction: !isAction });
        await this._execTask();
        ...
        if (currCard.type == CARD_TYPE.Equipment) { // 装备
            ...
            if (
                ...
                this._detectStatus(pidx, STATUS_TYPE.Usage, 'slot-destroy', { isOnlyCombatStatus: true, card: currCard });
                await this._execTask();
            }
        ...
        if (isInvalid) {
            ...
            assgin(player.summons, this._updateSummon(pidx, this._getSummonById(cardres.summon), player.summons, player, true, { destroy: true }));
            assgin(opponent.summons, this._updateSummon(pidx, [], opponent.summons, opponent, true, { destroy: true }));
            const stscmds: Cmds[] = [
    ...
    private async _changeTurn(pidx: number, isQuickAction: boolean, type: string, options: { isDieSwitch?: boolean } = {}) {
        if (this._hasNotDieSwitch()) await wait(() => this.needWait);
        ...
        const isChange = !isQuickAction && this._hasNotDieSwitch() &&
            (!isDieSwitch && !isOppoActionEnd ||
                isDieSwitch && this.players[pidx]?.phase < PHASE.ACTION_END);
        if (isChange) {
    ...
    private _doReset() {
        this.players.forEach(player => {
            ...
            if (rCombatStatus.length > 0) assgin(player.combatStatus, this._updateStatus(player.pidx, rCombatStatus, player.combatStatus, player, true, player.hidx));
        });
    }
    /**
     * 执行回合开始阶段检测
            ...
            this.players.forEach(player => {
                player.heros.forEach((h, hi, a) => {
                    ([...h.heroStatus, ...(h.isFront ? player.combatStatus : [])]).forEach(sts => {
                        if (sts.roundCnt > 0) --sts.roundCnt; // 持续回合数-1
                    });
                    assgin(a[hi].heroStatus, this._updateStatus(player.pidx, [], h.heroStatus, player, true, hi));
                });
                assgin(player.combatStatus, this._updateStatus(player.pidx, [], player.combatStatus, player, true, player.hidx));
    ...
    private _doDamage(pidx: number, damageVO: DamageVO, options: {
        isSwitch?: number, slotSelect?: number[], summonSelect?: number[], canAction?: boolean,
        isQuickAction?: boolean, atkname?: string, supportSelect?: number[], cmds?: Cmds[],
    } = {}) {
        if (damageVO == -1) return;
        let isDie = -1;
        let heroDie = -1;
        const { isSwitch = -1, slotSelect, summonSelect, supportSelect, canAction = true, isQuickAction = false, atkname = '', cmds = [] } = options;
        ...
        for (const p of this.players) {
            for (let offset = 0; offset < p.heros.length; ++offset) {
                ...
                if (h.hp > 0 || heal % 1 != 0) {
                    ...
                    if (willDamages[phidx][0] >= 0 || willDamages[phidx][1] > 0) {
        ...
        if (heroDie > -1) {
            if (isSwitch > -1 && this.players[isDie].heros[isSwitch].hp > 0) {
                isDie = -1;
                heroDie = -1;
            } else if (this._isWin() == -1) {
                this.isDieBackChange = !isQuickAction && this.players[isDie].phase == PHASE.ACTION;
                this.players[isDie].status = PLAYER_STATUS.DIESWITCH;
                this.players[isDie].UI.info = '请选择出战角色...';
                this.players[isDie ^ 1].UI.info = '等待对方选择出战角色......';
                this.players[isDie ^ 1].playerInfo.isKillByMyRound = true;
                const diehi = heroDie;
                // 击倒对方角色
                if (heroDie != pidx) this._detectSlot(pidx, 'kill');
                setTimeout(() => {
                    this.players[isDie].heros[diehi].isFront = false;
                    const tips = ['', ''];
                    tips[isDie] = '请选择出战角色';
                    tips[isDie ^ 1] = '等待对方选择出战角色';
                    this.emit(`${damageVO.dmgSource}-doDamage-heroDie`, isDie, { tip: tips, notPreview: true });
                }, 2400);
            }
        }
        this._doCmds(pidx, cmds);
        ...
        this.emit(`${damageVO.dmgSource}-doDamage-${atkname}`, pidx, {
            ...
            canAction,
            isQuickAction: isQuickAction && isDie == -1,
        });
    }
    /**
     * 检测治疗
    ...
    _detectHeal(pidx: number, heal: number[], players: Player[], options: {
        isExec?: boolean, notPreHeal?: boolean, isQuickAction?: boolean, supportCnt?: number[][],
    } = {}) {
        const { isExec = true, notPreHeal = false, isQuickAction = false,
            supportCnt = this.players.map(() => new Array(MAX_SUPPORT_COUNT).fill(0))
        } = options;
        const { heros } = players[pidx];
        if (heal.some(hl => hl > -1)) {
            if (!notPreHeal) this._detectStatus(pidx, STATUS_TYPE.Usage, 'pre-heal', { heal, players, supportCnt, isQuickAction, isExec });
            heal.forEach((h, hi) => {
                if (h > -1) this._detectSkill(pidx, 'heal', { hidxs: hi, players, heal, isQuickAction, isExec });
            });
            this._detectSlot(pidx, 'heal', { hidxs: allHidxs(heros), heal, players, supportCnt, isQuickAction, isExec });
            this._detectStatus(pidx, STATUS_TYPE.Usage, 'heal', { heal, players, supportCnt, isQuickAction, isExec });
    ...
    private _doDiscard(pidx: number, discards: Card[], options: {
        ...
        const { players = this.players, isAction = false, isExec = true,
            supportCnt = players.map(() => new Array(MAX_SUPPORT_COUNT).fill(0))
        } = options;
        const { heros, summons } = players[pidx];
        const tasks: { cmds: Cmds[], atkname: string }[] = [];
        let willHeals: number[] | undefined;
        for (const card of discards) {
            ...
            if (cardres.cmds) {
                const { willDamages = [] } = this._doCmds(pidx, cardres.cmds, { players, isAction, isExec: false, supportCnt });
                if (willDamages.length > 0) {
                    const atkname = `${card.name}(${card.entityId})`;
                    tasks.push({ cmds: cardres.cmds, atkname });
                    if (isExec) {
                        this.taskQueue.addTask(`doDamage-${atkname}`, [[() => {
                            const { willDamages = [], willHeals: [whl] = [], dmgElements = [], elTips = [], atkedIdx: tarHidx = -1, elrcmds = [] }
                            ...
                            this._doCmds(pidx, elrcmds);
                        }, 2250]]);
                    }
                }
            }
            if (cardres.summon) assgin(summons, this._updateSummon(pidx, this._getSummonById(cardres.summon), summons, players[pidx], isExec));
        }
        ...
        const { bWillHeals: stsheal, } = this._detectStatus(pidx, STATUS_TYPE.Usage, 'discard', {
            discards, heros, isExec, isOnlyExecSts: !isExec, isQuickAction: isCdt(!isAction, 2), supportCnt,
        });
        if (!willHeals) willHeals = stsheal;
        else mergeWillHeals(willHeals, stsheal);
        this._detectSupport(pidx, 'discard', { discard: discards.length, isExec, isQuickAction: !isAction, supportCnt });
    ...
    */
    ...
    private _detectSlot(pidx: number, otriggers: Trigger | Trigger[], options: {
        ...
        isExec?: boolean, usedDice?: number, isQuickAction?: boolean, getdmg?: number[], dmg?: number[],
        minusDiceSkillIds?: number[], minusDiceSkill?: number[][], cSlot?: Card, supportCnt?: number[][],
    } = {}) {
        const triggers: Trigger[] = [];
        ...
        const { players = this.players, summons = players[pidx].summons, heal, hcard, ehidx = players[pidx ^ 1].hidx,
            ...
            skid = -1, usedDice = 0, getdmg, dmg, cSlot,
            supportCnt = players.map(() => new Array(MAX_SUPPORT_COUNT).fill(0)),
            willSwitch = players.map(p => p.heros.map(() => false)) } = options;
        const player = players[pidx];
        ...
        const detectSlot = (slot: Card | null, hidx: number) => {
            if (slot == null || (taskMark && !slot.hasSubtype(taskMark[1]))) return;
            const tcmds: Cmds[] = [];
            const trgs: Trigger[] = [];
            ...
            for (const trigger of triggers) {
                ...
                if (hstop.length > 0) {
                    (slotres.hidxs ?? [eheros.findIndex(h => h.isFront)]).forEach(hi => {
                        assgin(eheros[hi].heroStatus, this._updateStatus(pidx ^ 1, hstop, eheros[hi].heroStatus, players[pidx ^ 1]));
                    });
                }
                if (cstop.length > 0) {
                    assgin(players[pidx ^ 1].combatStatus, this._updateStatus(pidx ^ 1, cstop, players[pidx ^ 1].combatStatus, players[pidx ^ 1]));
                }
            }
                ...
                if (!taskMark) {
                    ...
                    const slotHandle = async () => {
                        ...
                        if (!isAddTask || tcmds.length > 0) {
                            ...
                            if (cmdwh?.length) willHeals = cmdwh;
                        }
        ...
        let willHeals: number[] | undefined;
        if (!isExec) {
            const { willHeals: cmdheal } = this._doCmds(pidx, cmds, { players, getdmg, supportCnt, willSwitch, isExec: false });
    ...
    */
    private _detectStatus(pidx: number, otypes: StatusType | StatusType[], otrigger: Trigger | Trigger[], options: {
        ...
        players?: Player[], hidxs?: number[], skilltype?: SkillType, card?: Card, discards?: Card[], getdmg?: number[],
        ...
        const detectStatus = (stses: Status | Status[], group: StatusGroup, hidx: number, trigger: Trigger) => {
            ...
            for (const sts of stses) {
                ...
                const stsres = sts.handle(sts, {
                    ...
                    minusDiceSkill,
                    getdmg,
                    isExecTask: isExec && !isOnlyExecSts,
                    randomInt: this._randomInt.bind(this),
                    randomInArr: this._randomInArr.bind(this),
                });
                    ...
                    if (isExec && !isOnlyExecSts) {
                        if (
                            ...
                            if (stsres.isAddTask) {
                                ...
                                this._writeLog(`[${player.name}]${trigger}:${sts.name}${oCnt != -1 ? `.useCnt:${oCnt}->${sts.useCnt}` : ''}`, 'system');
                                this._doCmds(pidx, stscmds, { players, hidxs: [hidx], withCard: card, source: sts.id });
                            }
                        }
        ...
        if (isExec && !taskMark) {
            this.taskQueue.addStatusAtk(statusAtks.map(s => ({ ...s, isQuickAction: iqa })), isUnshift);
            this.taskQueue.addStatusAtk(statusAtksPre.map(s => ({ ...s, isQuickAction: iqa })), true);
        }
    ...
    private _detectSummon(pidx: number, ostate: Trigger | Trigger[],
        ...
        let willHeals: number[][] | undefined;
        if (!isExec) {
            ({ willHeals } = this._doCmds(pidx, smncmds, { supportCnt, willSwitch, isExec: false }));
        }
    ...
    private _detectSupport(pidx: number, ostates: Trigger | Trigger[],
        ...
        const detectSupport = (support: Support, stidx: number) => {
            for (const state of states) {
                ...
                if (isExec) {
                    const supportexecres = supportres.exec?.(clone(support), { isExecTask: false, isQuickAction, switchHeroDiceCnt }) ?? { isDestroy: false };
                    cmdsAll.push(...(supportexecres.cmds ?? []));
                    if (supportres.isNotAddTask) {
                        const supportexecres = supportres.exec?.(support, { isQuickAction, switchHeroDiceCnt }) ?? { isDestroy: false };
                        ...
                        if (!isExecTask) {
                            ...
                            const supportHandle = async () => {
                                ...
                                const supportexecres = supportres.exec?.(spt, { isQuickAction, switchHeroDiceCnt }) ?? { isDestroy: false };
                                ...
                                if (supportres.summon) assgin(player.summons, this._updateSummon(pidx, this._getSummonById(supportres.summon), player.summons, player, true));
                                ...
                                await delay(300);
                                if (supportexecres.cmds?.some(({ cmd }) => cmd == 'getCard')) await delay(700);
        ...
        if (isExec) assgin(player.supports, player.supports.filter((_, stidx) => !destroys.includes(stidx)));
            ...
            const { willHeals: cmdheal = [] } = this._doCmds(pidx, cmdsAll, { players, supportCnt, isAction: !isQuickAction, isExec: false });
    ...
    _doCmds(pidx: number, cmds?: Cmds[],
        ...
        willDamages?: number[][], dmgElements?: DamageType[], atkedIdx?: number, attackPreview?: Preview, elrcmds?: Cmds[],
        tasks?: { cmds: Cmds[], atkname: string }[],
    } {
        const { players = this.players } = options;
        const player = players[pidx];
        const opponent = players[pidx ^ 1];
        const { withCard, hidxs: chidxs, heal = 0, isExec = true, ahidx = player.hidx, ehidx = opponent.hidx,
            isAction = false, source = -1, damages, socket, isSummon = -1, trigger = '', getdmg = [],
            willSwitch = players.map(p => p.heros.map(() => false)),
            supportCnt = players.map(() => new Array(MAX_SUPPORT_COUNT).fill(0)) } = options;
        ...
        for (let i = 0; i < cmdLen; ++i) {
            const { cmd = '', cnt = 0, element, card, status: stsargs, isOppo, isAttach = false, mode = 0 } = cmds[i];
            ...
            if (cmd == 'useSkill') {
                if (isExec) {
                    this.taskQueue.addTask(`doCmd--useSkill:${cnt}`, [[() => {
                        this._useSkill(pidx, cnt, { withCard, isSwitch, isReadySkill: isAttach });
                    }]]);
                } else {
                ...
                if (sdir == 0) {
                    ...
                    nhidx = this._getFrontHero(cpidx, { players, offset: sdir }).hidx;
                }
                if (nhidx > -1) {
                    if (!isExec) {
                        ...
                        if (!cmds.some(({ cmd }) => cmd == 'useSkill') && !isOppo) {
                            this._useSkill(pidx, -1, { isPreview: true, isSwitch, otriggers: ['change-from', 'change-to'] });
                        }
                    } else {
                        cmds[i].hidxs = [nhidx];
                        this.taskQueue.addTask('doCmd--switch', [[() => {
                            this._switchHero(cpidx, nhidx, 'doCmd--switch', { socket, trigger })
                        }, 0, cnt]]);
                    }
                }
                ...
                const cards: Card[] = [];
                if (card) {
                    cards.push(...(Array.isArray(card) ? card : Array.from({ length: cnt }, () => clone(card)))
                ...
                if (cmd == 'getCard') {
                    ...
                    setTimeout(async () => {
                        for (let i = 0; i < cnt - restCnt; ++i) {
                            this._detectStatus(cpidx, STATUS_TYPE.Usage, 'getcard', { isExec, isQuickAction: isCdt(!isAction, 2), supportCnt });
                            this._detectSupport(cpidx ^ 1, 'getcard-oppo', { isExec, isQuickAction: !isAction, supportCnt });
                        }
                        if (isExec) await this._execTask();
                    }, 0);
                    if (willGetCard.length > 0 && isExec) {
                        ...
                        this.taskQueue.addTask(`getCard-p${cpidx}:${cardNames}`, [[async () => {
                            ...
                            p.UI.willGetCard.push(...willGetCard);
                            this._writeLog(`[${p.name}]抓${willGetCard.length}张牌【p${cpidx}:${cardNames}】`);
                ...
                if (cmd == 'addCard' && isExec) {
                    ...
                    this.taskQueue.addTask(`addCard-p${cpidx}:${cardStr}`, [[async () => {
                        ...
                        const scope = cmds[i].hidxs?.[0] ?? 0;
                        const isRandom = !isAttach;
                        const isNotPublic = mode == CMD_MODE.isNotPublic;
                        this._writeLog(`[${p.name}]将${isNotPublic ? `${cards.length}张牌` : `${cardStr}`}${scope == cards.length ? '' : isRandom ? '随机' : '均匀'}加入牌库${scope != 0 ? `${scope > 0 ? '顶' : '底'}${scope == cards.length ? '' : `${Math.abs(scope)}张`}` : ''}`);
                        ...
                        if (mode == CMD_MODE.AllHandCards) { // 弃置所有手牌
                            ...
                            while (discardCnt > 0) {
                                if (mode == CMD_MODE.Random) { // 弃置随机手牌
                                    ...
                                    const cdidx = this._randomInArr(unselectedCards.filter(c => c.cost + c.anydice == maxCost)).cidx;
                ...
                if (discards.length > 0) {
                    if (isExec) {
                        ...
                        this.taskQueue.addTask(`discard-p${pidx}:${cardNames}`, [[async () => {
                            const p = this.players[pidx];
                            if (mode == CMD_MODE.Random || mode == CMD_MODE.AllHandCards || mode == CMD_MODE.HighHandCard || card) { // 舍弃手牌
                            ...
                            this._writeLog(`[${p.name}]舍弃了${cardNames}`);
                            this.emit('doCmd--discard', pidx);
                            await delay(1500);
                            p.UI.willDiscard.forEach((_, i, a) => a[i] = []);
                            this.emit('doCmd--discard-after', pidx, { isQuickAction: !isAction });
                            this._doDiscard(pidx, discards!, { isAction });
                        }]]);
                    } else {
                        const { tasks: distasks, willHeals: disheal } = this._doDiscard(pidx, discards, { players, isAction, isExec: false, supportCnt });
                        tasks.push(...distasks);
                        if (disheal) {
            ...
            } else if (cmd == 'reroll' && (ndices?.length ?? player.dice.length) > 0 && isExec) {
                phase = [PHASE.DICE];
                player.rollCnt = cnt;
                player.UI.showRerollBtn = true;
            } else if (cmd == 'changeDice' && isExec) {
                assgin(player.dice, player.dice.map(() => element ?? DICE_COST_TYPE.Omni));
                ...
                notPreHeal[mode] ||= isAttach || cmd == 'revive';
                ...
                (hidxs ?? [(atkOppo1 ? opponent : player).hidx]).forEach((hidx, hi) => {
                    const {
                    ...
                    } = damages?.(atkOppo1, dmgcnt, element as DamageType | undefined, hidxs) ?? {};
        ...
        if (willDamages != undefined && dmgElement != undefined) {
            const { players: players1, willDamages: willDamage1, elTips: elTips1, dmgElements: dmgElements1, willHeal: willHeal1, skillcmds }
                = this._useSkill(pidx, -2, { isPreview: !isExec, players, isSummon, willDamages, dmgElement, atkedIdx: isCdt(!atkOppo, atkedIdx) });
        ...
        for (let fhidx = 0; fhidx < cheros.length; ++fhidx) {
            ...
            if (heroStatus && heroStatus[fhidx].length) {
                ...
                assgin(fhero.heroStatus, this._updateStatus(pidx, heroStatus[fhidx], fhero.heroStatus, player, true, fhidx));
            }
        }
        if (combatStatus) assgin(player.combatStatus, this._updateStatus(pidx, combatStatus, player.combatStatus, player, true, ahidx));
        for (let fhidx = 0; fhidx < ceheros.length; ++fhidx) {
            ...
            if (heroStatusOppo && heroStatusOppo[fhidx].length) {
                ...
                assgin(fhero.heroStatus, this._updateStatus(pidx ^ 1, heroStatusOppo[fhidx], fhero.heroStatus, opponent, true, fhidx));
            }
        }
        if (combatStatusOppo) assgin(opponent.combatStatus, this._updateStatus(pidx ^ 1, combatStatusOppo, opponent.combatStatus, opponent, true, ehidx));
        return {
            cmds, ndices, phase, heros: cheros, eheros: ceheros, willHeals, changedEl, isSwitch, isSwitchOppo, discards, elrcmds,
        ...
        for (const sid of skids) {
            ...
            const { willHp, willAttachs, summonCnt, supportCnt, willSummons, willSwitch }
                = this._useSkill(pidx, sid, {
                    ...
                    willDamages,
                    dmgElement,
                    otriggers: isCdt(sid == -1, ['change-from', 'change-to']),
                    atkedIdx,
                });
    ...
    private _previewCard(pidx: number) {
        ...
        for (const cidx in handCards) {
            ...
            const player = players[pidx];
            const { isValid: diceValid, diceSelect, heroCanSelect = [], supportCnt, attackPreview: atkPreview = {} }
     ...
     * @returns 合并后状态
     */
    _updateStatus(pidx: number, nStatus: Status[], oStatus: Status[], player: Player, isExec = false, hidx: number = -1, ohidx: number = -1) {
        const oriStatus: Status[] = clone(oStatus);
        const newStatus: Status[] = clone(nStatus);
        const { combatStatus, heros } = player;
        ...
        newStatus.forEach(sts => {
            ...
            if (cstIdx > -1) { // 重复生成状态
                ...
                const { isFallAtk = false } = sts.handle(sts, { heros, hidx, trigger: 'enter' });
    ...
    private _updateSummon(
        ...
        newSummon.forEach(smn => {
            ...
            if (csmnIdx > -1) { // 重复生成召唤物
                ...
                if (smnres.rCombatStatus) {
                    const nsts = this._updateStatus(pidx, this._getStatusById(smnres.rCombatStatus), combatStatus, player, isExec, hidx);
                    assgin(combatStatus, nsts);
                }
            }
        });
        ...
        return oriSummon.filter(smn => {
            if (smn.statusId > 0) { // 召唤物有关联状态
                ...
                const nsts = this._updateStatus(pidx, [smnStatus], combatStatus, player, isExec, hidx);
                assgin(combatStatus, nsts);
            }
     ...
     */
    async _execTask() {
        if (this.taskQueue.isExecuting || this.taskQueue.isTaskEmpty() || !this._hasNotDieSwitch()) return;
        this.taskQueue.isExecuting = true;
        while (this._hasNotDieSwitch() && !this.taskQueue.isTaskEmpty() && this.taskQueue.isExecuting) {
            ...
            if (taskType.startsWith('statusAtk-')) {
                const isExeced = await this.taskQueue.execTask(taskType, [[() => this._doStatusAtk(args as StatusTask)]], isPriority);
                if (!isExeced) {
    ...
    */
     ...
     * @returns 角色区域数组
     */
    private _getHeroField(pidx: number, options: { players?: Player[], hidx?: number } = {}) {
        const { players = this.players } = options;
