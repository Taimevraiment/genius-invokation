...
export default class GeniusInvokationRoom {
    ...
    emit(flag: string, pidx: number, options: {
        ...
        const { socket, tip = '', actionInfo = '', damageVO = -1, notPreview = false, isActionInfo = false, canAction = true,
            ...
            isQuickAction = false, isChange = false, hasReadyskill = false } = options;
        this.players.forEach(p => {
            ...
            assgin(p.summons, this._updateSummon(p.pidx, [], p.summons, p));
    ...
    private _checkSkill(pidx: number, skid: number, isSwitch: number = -1) {
        ...
        if (costType == COST_TYPE.Same && elementDiceCnt > 0) {
    ...
    private _useSkill(pidx: number, skid: number | SkillType, options: {
        ...
        if (skillres.summonPre) assgin(player().summons, this._updateSummon(pidx, this._getSummonById(skillres.summonPre), aSummons(), player(), isExec));
        if (skillres.heal) {
        ...
        if (dmgEl && oWillDamages) {
            const { willDamages: willDamage1, willAttachs: willAttachs1, dmgElements: dmgElements1,
                ...
                aWillHeals: ahealres, bWillHeals: bhealres, isQuickAction: iqa1, isFallAtk: ifa1
            } = this._calcDamage(
                pidx,
                dmgEl,
                oWillDamages,
            ...
            if (skillres.summon) assgin(aSummons(), this._updateSummon(pidx, this._getSummonById(skillres.summon), aSummons(), player(), isExec));
    ...
    private _calcDamage(
        ...
        options: {
            ...
            bWillHeals?: number[], elrcmds?: Cmds[][], atriggers?: Trigger[][], etriggers?: Trigger[][], usedDice?: number,
            dmgElements?: DamageType[], willSwitch?: boolean[][], elTips?: [string, PureElementType, PureElementType][],
        ...
        const { isAttach = false, isSummon = -1, isSwirl = false, isExec = true, multiDmg = 0, skid = -1, isAtkSelf = 0,
            ...
            willSwitch = players.map(p => p.heros.map(() => false)), isQuickAction = false,
        } = options;
        ...
        const detectStatus = (trgs: Trigger[], hi: number, isSelf: boolean) => {
            ...
            if (stsres.nsummons.length > 0) {
                assgin(res.players[cpidx].summons, this._updateSummon(cpidx, stsres.nsummons, res.players[cpidx].summons, res.players[cpidx], isExec));
            }
        ...
        assgin(res.players[pidx].summons, this._updateSummon(pidx, [], asummons, res.players[pidx], isExec));
        if (!isSwirl && isExec) {
    ...
    private async _changeTurn(pidx: number, isQuickAction: boolean, type: string, options: { isDieSwitch?: boolean } = {}) {
        if (this._hasNotDieSwitch()) await wait(() => this.needWait, { maxtime: 2e5 });
    ...
    async _doStatusAtk(stsTask: StatusTask) {
        ...
        if (!stsres.damage && !stsres.heal && stsres.cmds?.every(({ cmd }) => cmd != 'revive')) {
    ...
    */
    ...
    private _detectSlot(pidx: number, otriggers: Trigger | Trigger[], options: {
        ...
        const { players = this.players, summons = players[pidx].summons, heal, hcard, ehidx = players[pidx ^ 1].hidx,
            ...
            skid = -1, sktype, usedDice = 0, getdmg, dmg, cSlot, isEffectStatus = false,
            supportCnt = players.map(() => new Array(MAX_SUPPORT_COUNT).fill(0)),
            ...
            for (const trigger of triggers) {
                ...
                if (isEffectStatus) {
                    if (heroStatus.length > 0) {
                        assgin(heros[hidx].heroStatus, this._updateStatus(pidx, heroStatus, heros[hidx].heroStatus, players[pidx]));
                    }
                    if (combatStatus.length > 0) {
                        assgin(players[pidx].combatStatus, this._updateStatus(pidx, combatStatus, players[pidx].combatStatus, players[pidx]));
                    }
                }
    ...
    */
    private _detectStatus(pidx: number, otypes: StatusType | StatusType[], otrigger: Trigger | Trigger[], options: {
        ...
        const { isExec = true, isOnlyFront = false, players = this.players, hasDmg = false,
            ...
            supportCnt = players.map(() => new Array(MAX_SUPPORT_COUNT).fill(0)), slotsDestroy,
            willSwitch = players.map(p => p.heros.map(() => false)) } = options;
        let addDiceHero = 0;
        let minusDiceHero = 0;
        let isInvalid = false;
                    ...
                    if (supportres.isNotAddTask) {
                        ...
                        if (!isExecTask) {
                            ...
                            const supportHandle = async () => {
                                ...
                                if (supportres.summon) assgin(player.summons, this._updateSummon(pidx, this._getSummonById(supportres.summon), player.summons, player));
            ...
            if (cmd == 'useSkill') {
                ...
                if (cmd == 'addCard' && isExec) {
                    ...
                    this.taskQueue.addTask(`addCard-p${cpidx}:${cardStr}`, [[async () => {
                        ...
                        this._writeLog(`[${p.name}]将${cards.length}张牌${isNotPublic ? `【p${cpidx}:${cardStr}】` : ''}${Math.abs(scope) == cards.length ? '' : isRandom ? '随机' : '均匀'}加入牌库${scope != 0 ? `${scope > 0 ? '顶' : '底'}${Math.abs(scope) == cards.length ? '' : `${Math.abs(scope)}张`}` : ''}`);
        ...
        for (const skillId of skids) {
            ...
            const { willHp, willAttachs, summonCnt, supportCnt, willSummons, willSwitch }
                = this._useSkill(pidx, skillId, {
                    ...
                    willDamages,
                    dmgElement,
                    otriggers: isCdt(skillId == -1, ['change-from', 'change-to']),
                    atkedIdx,
                });
            const preview: Preview = {
                ...
                skillId,
                diceSelect: isCdt(skillId > -1, () => this._checkSkill(pidx, skillId, isSwitch).map(v => (skill?.isForbidden ?? true) ? false : v)),
                willHp,
                willAttachs,
    ...
    private _updateSummon(
        pidx: number, nSummon: Summon[], oSummon: Summon[], player: Player, isExec = true,
