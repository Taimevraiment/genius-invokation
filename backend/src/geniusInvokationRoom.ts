...
import {
    ActionData, AtkTask, CalcAtkRes, Card, Cmds, Countdown, DamageVO, Env, Hero, LogType, MinusDiceSkill, PickCard,
...
export default class GeniusInvokationRoom {
        ...
        try {
            ...
            const _serverDataVO = (vopidx: number, tip: string | string[]) => {
                const log = this.log.map(lg => {
                    ...
                    if (cpidxs.length == 0) return { content: lg.content.replace(/【[^【】]+】/g, ''), type: lg.type }
    ...
    private async _switchHero(pidx: number, hidx: number, flag: string,
        ...
        if (isDieSwitch) { // 被击倒后选择出战角色
            ...
            const { cmds: supportcmd } = this._detectSupport(pidx, 'active-switch', { hidx, isQuickAction });
    ...
    private _calcDamage(
        ...
        let isAttachElement: 'attach' | 'consume' | 'none' = 'none';
        if (dmgElement != DAMAGE_TYPE.Pierce && (res.willDamages[getDmgIdx][0] >= 0 || isAttach)) {
            ...
            if ( // 没有元素反应(不含冰草共存)
                ...
                const [attachElement] = efhero.attachElement;
                ...
                if (dmgElement == ELEMENT_TYPE.Anemo) { // 扩散
                    ...
                    etriggers[dmgedHidx].push('get-Swirl');
                    ...
                    etriggers[dmgedHidx].push('get-Crystallize');
                } else {
                    ...
                    if (hasEls(ELEMENT_TYPE.Cryo, ELEMENT_TYPE.Dendro)) { // 冰草共存
                        ...
                        etriggers[dmgedHidx].push('get-Vaporize');
                        ...
                        etriggers[dmgedHidx].push('get-Melt');
                        ...
                        etriggers[dmgedHidx].push('get-ElectroCharged');
                        ...
                        etriggers[dmgedHidx].push('get-Superconduct');
                        ...
                        etriggers[dmgedHidx].push('get-Frozen');
                        ...
                        etriggers[dmgedHidx].push('get-Bloom');
                        ...
                        etriggers[dmgedHidx].push('get-Overload');
                        ...
                        etriggers[dmgedHidx].push('get-Burning');
                        ...
                        etriggers[dmgedHidx].push('get-Quicken');
                    }
                }
                if (isElDmg) {
        ...
        for (let i = 0; i < ahlen; ++i) {
            ...
            if (hfieldres.nsummons.length > 0) {
                this._updateSummon(atkPidx, hfieldres.nsummons, res.players, isExec, { isSummon, supportCnt });
            }
        ...
        if (isAttachElement == 'attach') efhero.attachElement.push(dmgElement as PureElementType);
        else if (isAttachElement == 'consume') efhero.attachElement.shift();
        ...
        if (dmgElement != DAMAGE_TYPE.Pierce && res.willDamages[getDmgIdx][0] > 0) {
            ...
            const { restDmg: nrestdmg = -1, pdmgs } = this._detectSlotAndStatus(dmgedPidx, 'reduce-dmg', {
    ...
    private async _useCard(pidx: number, cardIdx: number, diceSelect: boolean[],
        options: {
            ...
            getcard?: Card, isQuickAction?: boolean,
        } = {}) {
    ...
    private async _doDamage(pidx: number, damageVO: DamageVO, options: {
        ...
        heroSelect?: number[], isActionInfo?: boolean,
    } = {}) {
        ...
        const { slotSelect, summonSelect, supportSelect, heroSelect, canAction = true, isQuickAction = false,
            atkname = '', cmds = [], skid = -1, isActionInfo = true } = options;
        ...
        this.emit(`${damageVO.dmgSource}-doDamage-${atkname}`, pidx, {
            damageVO,
            isActionInfo: isActionInfo && ['skill', 'status', 'card'].includes(damageVO.dmgSource),
            slotSelect,
            summonSelect,
            supportSelect,
            heroSelect,
    ...
    private _detectHeal(pidx: number, heal: number[], players: Player[], options: {
        ...
        if (heal.some(hl => hl > -1)) {
            ...
            const { tasks: ht } = this._detectSlotAndStatus(pidx, 'heal', { types: STATUS_TYPE.Usage, hidxs: allHidxs(heros), players, heal, isQuickAction, supportCnt, source, isExec, isOnlyExec: !isExec });
            tasks.push(...ht);
    ...
    */
    private _detectSkill(pidx: number, otrigger: Trigger | Trigger[],
        ...
        const { players = this.players, isExec = true, getdmg = [], dmg = [], heal = [], discards = [], card, source, sourceHidx,
            isExecTask = false, heros = players[pidx].heros, eheros = players[pidx ^ 1].heros, cskid = -1, energyCnt, isOnlyExec } = options;
        let { hidxs = allHidxs(heros), isQuickAction = false, type = [], restDmg } = options;
        ...
        return { isQuickAction, cmds, isTriggered, task, restDmg }
    }
    /**
     * 检测装备发动
     * @param pidx 玩家序号
     ...
     *          nsummons 新出的召唤物, isQuickAction 是否为快速攻击, supportCnt 支援区的变化数, restDmg 减伤
     */
    private _detectSlot(pidx: number, otriggers: Trigger | Trigger[], options: {
        ...
        let { switchHeroDiceCnt = 0, minusDiceCard = 0, hidxs = [player.hidx], cSlot,
            isQuickAction = false, hcardsCnt = player.handCards.length, restDmg } = options;
        let addDmg = 0;
        ...
        return {
            ...
            supportCnt, heroStatus, combatStatus, nsummons, isQuickAction, minusDiceCard, cmds, task, restDmg,
        }
    }
    /**
     * 状态效果发动
    ...
    */
    private _detectStatus(pidx: number, otypes: StatusType | StatusType[], otrigger: Trigger | Trigger[], options: {
        ...
        let { isQuickAction = false, switchHeroDiceCnt = 0, minusDiceCard = 0, restDmg } = options;
        const { isExec = true, isOnlyFront, players = this.players, hasDmg,
                            ...
                            if (
                                ...
                                if (stsres.isAddTask) {
                                    ...
                                    if (!taskMark) {
                                        ...
                                        const drawcard = trigger == 'drawcard' ? `(${hcard?.entityId ?? -1})` : '';
                                        this.taskQueue.addTask(`status-${sts.name}(${sts.entityId}):${trigger}${drawcard}`, args, { isUnshift, isDmg: true });
                                    } else {
                                        const statusHandle = async () => {
                                            ...
                                            this._writeLog(`[${player.name}]${trigger}:${sts.name}${sts.useCnt != -1 ? `.useCnt:${useCntChange}` : ''}${curStatus.perCnt != -1 ? `.perCnt:${perCntChange}` : ''}`, 'system');
                                            if (!curStatus.hasType(STATUS_TYPE.Hide)) this._writeLog(`[${this.players[pidx].name}][${curStatus.name}]发动${oCnt != curStatus.useCnt ? ` ${useCntChange}` : ''}`, stsres.notLog ? 'system' : stsres.notInfo ? 'log' : 'info');
                                            ...
                                            this.emit(flag, pidx, {
                                                isActionInfo: !stsres.notInfo && !stsres.notLog,
                                                isQuickAction,
        ...
        return {
            ...
            nsummons, aWillHeals, bWillHeals, supportCnt, pdmgs, cmds, isFallAtk: stsFallAtk, restDmg,
        }
    }
    /**
     * 检测角色区域
    ...
    private _detectSlotAndStatus(pidx: number, otrigger: Trigger | Trigger[], options: {
        ...
        for (const hidx of hidxs) {
            ...
            for (const field of heroField) {
                if ('group' in field) {
                    ...
                    const { switchHeroDiceCnt: slotSwitchHeroDiceCnt, addDmg: slotAddDmg, isQuickAction: slotIqa,
                        ...
                        nsummons: slotNSummons, minusDiceCard: slotMinusDiceCard, cmds: slotCmds, tasks: slottasks }
            ...
            ({ minusDiceCard, switchHeroDiceCnt, restDmg } = options);
        ...
        return {
            minusDiceCard, switchHeroDiceCnt, isInvalid, cmds, aWillHeals, bWillHeals, isQuickAction, restDmg,
            ...
            for (const summon of summons) {
                ...
                if (summonres.isNotAddTask) {
                    if (summonres.exec && isExec) {
                        ...
                        const { changedEl } = this._doCmds(pidx, cmds, { players, isExec, isPriority: true });
    ...
    private _detectSupport(pidx: number, ostates: Trigger | Trigger[],
            ...
            for (const state of states) {
                ...
                if (isExec) {
                    if (supportres.isNotAddTask) {
                        const supportexecres = supportres.exec?.(support, {}) ?? {};
        ...
        return { isQuickAction, cmds, exchangeSupport, switchHeroDiceCnt, supportCnt, minusDiceCard, willSummons, willHeals, task, tasks }
    }
    /**
     * 获取骰子
     * @param pidx 玩家序号
    ...
    private _doCmds(pidx: number, cmds?: Cmds[],
        options: {
            ...
            energyCnt?: number[][], isOnlyGetWillHeal?: boolean,
        } = {}
    ): {
        cmds?: Cmds[], ndices?: DiceCostType[], phase?: Phase, heros?: Hero[], eheros?: Hero[], willHeals?: number[][],
        changedEl?: ElementType, isSwitch?: number, isSwitchOppo?: number, willSwitch?: boolean[][],
        ...
        const { withCard, hidxs: chidxs, heal = 0, isExec = true, ahidx = player.hidx, ehidx = opponent.hidx,
            ...
            supportCnt = INIT_SUPPORTCNT(), isPriority, isUnshift, isOnlyGetWillHeal = false,
            ...
            if (cmd == 'useSkill') {
                ...
                if (cmd == 'getCard') {
                    ...
                    if (willGetCard.length > 0) {
                        ...
                        for (let i = 0; i < count - restCnt - 1; ++i) {
                        ...
                        for (const getcard of getcards) {
                            ...
                            if (isExec) {
                                this.taskQueue.addTask(`doCmd--getCard-p${cpidx}-${getcard.name}:getcard`, [[async () => {
                ...
                if (discards.length > 0) {
                    ...
                    if (isExec) {
                        ...
                        this.taskQueue.addTask(`doCmd--discard-p${cpidx}:${cardNames}`, [[async () => {
                            ...
                            this._writeLog(`[${p.name}]${isDiscard ? '舍弃了' : '被夺取了'}${cardNames}`, isCdt(isAttach, 'system'));
                ...
                (ohidxs ?? [player.hidx]).forEach((hidx, hi) => {
                    ...
                    const { players: players1, elTips: elTips1, willAttachs: willAttachs1 }
                        = this._calcDamage(pidx, dmgel, [], hidx, players, { isAttach: true, elTips, isExec });
        ...
        return {
            cmds, phase, heros: cheros, eheros: ceheros, willHeals, changedEl, isSwitch, isSwitchOppo,
    ...
    private _previewSkill(pidx: number, skid?: number, options: {
        ...
        for (const skillId of skids) {
            ...
            if (summonSelects.length > 0) summonSelects.unshift(-1);
    ...
    */
     ...
     * @param isExec 是否执行
     */
    private _calcHeroSwitch(pidx: number, tohidx: number, fromhidx: number, isExec = false) {
        const tasks: AtkTask[] = [];
