...
export default class GeniusInvokationRoom {
    ...
    emit(flag: string, pidx: number, options: {
        ...
        const { socket, tip = '', actionInfo = '', damageVO = -1, notPreview = false, isActionInfo = false,
            slotSelect = [], heroSelect = [], statusSelect = [], summonSelect = [], notUpdate = false } = options;
        ...
        this.players.forEach(p => {
            ...
            if (p.UI.willGetCard.length > 0) {
                if (this.isGettingCard != null) clearTimeout(this.isGettingCard);
                else this._writeLog(`[${p.name}]获得了手牌${p.UI.willGetCard.map(c => `[${c.name}]`).join('')}`);
            ...
            if (p.UI.willDiscard[0].length + p.UI.willDiscard[1].length > 0) {
                if (this.isDiscarding != null) clearTimeout(this.isDiscarding);
                else this._writeLog(`[${p.name}]舍弃了${p.UI.willDiscard.map(cs => cs.map(c => `[${c.name}]`).join('')).join('')}`);
    ...
    start(pidx: number, flag: string) {
        ...
        this.players.forEach(p => {
            ...
            this._writeLog(p.pile.reduce((a, c) => a += `[${c.name}]`, `[${p.name}]牌库：`), true)
        });
        this.emit(flag, pidx);
    }
    /**
     ...
     * 使用技能
     * @param pidx 玩家序号
     * @param skidx 选组技能的索引idx/准备技能id -1切换角色
    ...
    private _useSkill(pidx: number, skidx: number, options: { isPreview?: boolean, withCard?: Card, isSwitch?: number, isReadySkill?: boolean, otriggers?: Trigger | Trigger[] } = {}) {
        ...
        const dmgElements: DamageType[] = new Array(ehlen + ahlen).fill(DAMAGE_TYPE.Physical);
        ...
        if (skidx > -1) {
            if (skillres.atkOffset != undefined) dmgedHidx = this._getFrontHero(pidx ^ 1, { offset: skillres.atkOffset }).hidx;
            ...
            if (skillres.heal) {
                const { willHeals } = this._doCmds(pidx, [{ cmd: 'heal', cnt: skillres.heal, hidxs: skillres.hidxs ?? [hidx] }], { players, isNotAddTask: true, isExec });
            ...
            assgin(dmgElements, dmgElements1);
        ...
        const calcAtk = (res: any, type: string, stsId: number, ahidx: number, ehidx: number, isSkill = false, isSelf = 0) => {
            ...
            if (res?.heal) {
                ...
                if (type.includes('Status')) {
                    if (res.heal) {
                        ({ willHeals } = this._doCmds(cpidx, [{ cmd: 'heal', cnt: res.heal, hidxs: res.hidxs ?? [atkHidx] }], { players: bPlayers }));
                    }
                } else {
                    ({ willHeals } = this._doCmds(cpidx, res?.cmds, { hidxs: [atkHidx], players: bPlayers }));
                }
            ...
            for (let i = 0; i < dmgedHlen; ++i) bWillAttach[i + (cpidx ^ +!res.isSelf) * (isSelf ? ehlen : ahlen)].push(willAttachs3[i]);
            ...
            if (isSelf) {
                ...
                if (statusAtks.length > 0) isSwitchAtk = true;
                doAfterStatus(bPlayers[pidx ^ 1].heros[ehidx].heroStatus, ehidx, STATUS_GROUP.heroStatus, etriggers3[ehidx], ahidx, ehidx, 0, true);
                doAfterStatus(bPlayers[pidx ^ 1].combatStatus, ehidx, STATUS_GROUP.combatStatus, etriggers3[odmgedHidx], ahidx, ehidx, 0, true);
            }
            return false;
        }
            ...
            for (const sts of status) {
                ...
                for (const state of trgs) {
                    ...
                    if (sts.hasType(STATUS_TYPE.Attack) && (stsres.damage || stsres.pdmg || stsres.heal)) {
                        ...
                        if (calcAtk(stsres, willKill ? 'die' : (['hero', 'combat'][group] + 'Status'), sts.id, ahidx, ehidx, state.includes('skill'), isSelf)) continue;
                    }
                }
            }
            return { atkStatus }
        ...
        const aswhidx = isSwitchSelf(skillcmds) ? swc : skidx == -1 && dtriggers.includes('change-to') ? isSwitch : ahidx();
        ...
        if (skidx > -1) {
            ...
            for (let i = 0; i < ehlen; ++i) {
                ...
                const h = bPlayers[pidx ^ 1].heros[hi];
                ...
                if (i == 0) {
                    const { atkStatus: atkcst } = doAfterStatus(bPlayers[pidx ^ 1].combatStatus, hi, STATUS_GROUP.combatStatus, afterESkillTrgs[hi], aswhidx, eswhidx);
            ...
            for (const smn of bPlayers[pidx].summons) {
                ([`after-skilltype${skill.type}`, 'after-skill'] as Trigger[]).forEach(trg => {
                    ...
                    if (smnres) calcAtk(smnres, 'summon', smn.id, aswhidx, eswhidx, true, 1);
                });
            }
        }
        if (isSwitchOppo(skillcmds)) {
            ...
            if (statusAtks.length > 0) isSwitchAtk = true;
            doAfterStatus(bPlayers[pidx ^ 1].heros[odmgedHidx].heroStatus, odmgedHidx, STATUS_GROUP.heroStatus, ['change-from', 'change-to'], aswhidx, eswhidx, 0, true);
            doAfterStatus(bPlayers[pidx ^ 1].combatStatus, odmgedHidx, STATUS_GROUP.combatStatus, ['change-from', 'change-to'], aswhidx, eswhidx, 0, true);
        }
        ...
        if (!isExec) {
            willHp = willHp.map((_, i) => {
                ...
                if (res + hero.hp <= 0) {
                    if (hero.talentSlot?.hasTag(CARD_TAG.NonDefeat)) {
                        if (reviveSlot[whhidx] > 0) {
                            return reviveSlot[whhidx] - hero.hp - 0.3;
                        }
                    }
            ...
            const { isDie } = this._doDamage(pidx, {
                dmgSource: 'skill',
                atkPidx: pidx,
                atkHidx: aHeros()[player().hidx].hidx,
                tarHidx: aWillDamages.slice((pidx ^ 1) * ahlen, (pidx ^ 1) * ahlen + ehlen).some(([d, p]) => d > -1 || p > 0) ? dmgedHidx : -1,
                willDamages: aWillDamages,
                dmgElements,
                elTips: aElTips,
                willHeals: aWillHeal,
            }, { isSwitch });
            const hasSwitch = skillcmds.some(({ cmd }) => cmd.includes('switch')) || eskillcmds.some(({ cmd }) => cmd.includes('switch'));
            setTimeout(async () => {
                if (!this.taskQueue.isTaskEmpty()) await delay(hasSwitch ? 1500 : 2500);
                await this._execTask();
                await wait(() => !this.taskQueue.isExecuting);
                this._changeTurn(pidx, isQuickAction, 'useSkill', { isDie });
            }, hasSwitch ? 1000 : 0);
        }
        return { willHp, willAttachs: bWillAttach, summonCnt, supportCnt, willSummons, willSwitch }
    }
    /**
     * 计算伤害
    ...
    private _calcDamage(
        ...
        options: {
            ...
            elTips?: [string, PureElementType, PureElementType][], willAttachs?: ElementType[], multiDmg?: number, isAtkSelf?: number, withCard?: Card,
        ...
        const { isAttach = false, isSummon = [-1, -1], isSwirl = false, isExec = true, multiDmg = 0, skidx = -1, isAtkSelf = 0,
            ...
            elrcmds = [[], []], usedDice = 0, dmgElements = new Array<DamageType>(ehlen + ahlen).fill(DAMAGE_TYPE.Physical),
        ...
        if (!isSwirl) {
            ...
            else if (willDamages.length < ehlen + ahlen) {
        ...
        const getDmgIdxOffset = isAtkSelf ? ehlen * pidx : ahlen * epidx;
        const aGetDmgIdxOffset = ehlen * pidx;
            ...
            if ( // 没有元素反应(不含冰草共存)
                ...
                if (dmgElement == ELEMENT_TYPE.Anemo && elTypes.includes(attachElement)) { // 扩散
                    ...
                    if (hasEls(ELEMENT_TYPE.Cryo, ELEMENT_TYPE.Dendro)) { // 冰草共存
                        ...
                        assgin(res.players[cpidx].summons, this._updateSummon(cpidx, [this.newSummon(115)], res.players[cpidx].summons, res.players[cpidx], isExec, { isSummon: isSummon[0] }));
        ...
        if (!isAttach) {
            ...
            if (slotSummons.length > 0) {
                assgin(res.players[pidx].summons, this._updateSummon(pidx, slotSummons, res.players[pidx].summons, res.players[pidx], isExec, { isSummon: isSummon[0] }));
            }
        ...
        const detectStatus = (trgs: Trigger[], hi: number, isSelf: boolean) => {
            ...
            res.elrcmds[+!isSelf].push(...stsres.taskcmds);
        }
        if (!isSwirl) {
        ...
        assgin(players[epidx].summons, this._updateSummon(epidx, [], esummons, players[epidx]));
    ...
    private async _useCard(pidx: number, cardIdx: number, diceSelect: boolean[], socket: Socket,
        ...
        if (isInvalid) {
            ...
            player.summons = this._updateSummon(pidx, cardres.summon ?? [], player.summons, player);
            opponent.summons = this._updateSummon(pidx, [], opponent.summons, opponent);
            const stscmds: Cmds[] = [
        ...
        if (currCard.hasSubtype(CARD_SUBTYPE.Action) && !isUseSkill) {
            setTimeout(() => this._changeTurn(pidx, currCard.type == CARD_TYPE.Event || isInvalid, 'useCard'), 0);
        }
    ...
    private async _changeTurn(pidx: number, isQuickAction: boolean, type: string, options: { dieChangeBack?: boolean, isDie?: boolean } = {}) {
        await this._doActionAfter(pidx);
        ...
        const isTaskEmpty = this.taskQueue.isTaskEmpty();
        if (type == 'switchHero') { // 如果有速切或对方结束回合或有额外攻击则不转变回合
            canChange = isTaskEmpty && (!dieChangeBack && !isOppoActionEnd && !isQuickAction ||
            ...
            canChange = !isOppoActionEnd && isTaskEmpty && !isQuickAction;
            if (['doSummon', 'doStatus', 'doSlot', 'doSkill', 'useCard'].includes(type)) timeout = 600;
        } else if (type == 'endPhase') {
            canChange = isTaskEmpty;
            timeout = 600;
        }
        ...
        setTimeout(async () => {
            if (canChange) {
                ...
                if ((ephase > PHASE.ACTION || isQuickAction) && isTaskEmpty) {
                    this.players[this.currentPlayerIdx].canAction = true;
                }
            }
            await this._doActionStart(this.currentPlayerIdx);
            await delay(1200);
            const tip = isCdt(!isQuickAction && !isDie, canChange ? '{p}回合开始' : '继续{p}回合');
            this.emit('changeTurn-' + type + '-canChange:' + canChange, pidx, { tip });
        }, timeout);
    }
    /**
     * 玩家选择行动前
    ...
    private _doReset() {
        this.players.forEach(player => {
            ...
            player.heros.forEach(h => { // 重置技能
                h.skills.forEach((skill, skidx) => skill.handle({ reset: true, hero: h, skidx }))
            });
    ...
    private _doStatusDestroy(pidx: number, status: Status) {
        this._detectStatus(pidx, STATUS_TYPE.Attack, 'status-destroy', { cStatus: status, isUnshift: true });
    }
    /**
     * 召唤物消失时发动
     * @param pidx 玩家序号
    ...
    private _doSummonDestroy(pidx: number) {
        this._detectStatus(pidx, [STATUS_TYPE.Attack, STATUS_TYPE.Usage], 'summon-destroy', { isUnshift: true });
    }
    /**
     * 支援物消失时发动
     * @param pidx 玩家序号
     ...
     */
    private _doDamage(pidx: number, damageVO: DamageVO, options: { isSwitch?: number, slotSelect?: number[], summonSelect?: number[] } = {}) {
        if (damageVO == -1) return { isDie: false };
        let isDie = -1;
        const { isSwitch = -1, slotSelect, summonSelect } = options;
        ...
        this.emit(damageVO.dmgSource + '-doDamage', pidx, { damageVO, isActionInfo, slotSelect, summonSelect, notUpdate: damageVO.dmgSource == 'status' });
     ...
     * @param pidx 玩家序号
     * @param cards 弃牌
     * @param options heros 角色组, isAction 是否为战斗行动, isExec 是否执行
     */
    private _doDiscard(pidx: number, cards: Card[], options: { heros?: Hero[], isAction?: boolean, isExec?: boolean } = {}) {
        const { heros = this.players[pidx].heros, isAction = false, isExec = false } = options;
        const summons: Summon[] = [];
        const cmds: Cmds[] = [];
        for (const card of cards) {
            const cardres = card.handle(card, { trigger: 'discard', isExec });
            if (this._hasNotTriggered(cardres.trigger, 'discard')) continue;
            const { cmds: cardcmds = [] } = this._doCmds(pidx, cardres.cmds, { isExec });
            cmds.push(...cardcmds);
            summons.push(...(cardres.summon ?? []));
        }
        this._detectSkill(pidx, 'discard', { heros, discards: cards, isExec });
        this._detectStatus(pidx, STATUS_TYPE.Usage, 'discard', { discards: cards, heros, isExec, isQuickAction: isCdt(!isAction, 2) });
        this._detectSupport(pidx, 'discard', { discard: cards.length, isExec, isQuickAction: !isAction });
        return { cmds, summons }
    }
    /**
     * 状态攻击
    ...
    async _doStatusAtk(stsTask: StatusTask, isPriority: boolean) {
        ...
        const atkStatus = () => (group == STATUS_GROUP.heroStatus ? aheros[ahidx].heroStatus : combatStatus).find(sts => sts.entityId == entityId)!;
        const atkedIdx = isSelf ? ahidx : eFrontIdx;
        if (atkStatus() == undefined) return true;
        ...
        let { willDamages, dmgElements, players: players1, elrcmds, elTips, isQuickAction: iqa }
            = this._calcDamage(
                ...
                atkedIdx,
                this.players,
                { isAtkSelf: isSelf, skidx: isCdt(trigger.includes('skill'), -2) }
            );
        isQuickAction ||= iqa;
        ...
        this._doCmds(pidx ^ +!isSelf, elrcmds[1], { heros: isSelf ? aheros : eheros });
        if (stsres.heal) cmds.push({ cmd: 'heal', cnt: stsres.heal, hidxs: stsres.hidxs });
        if (cmds.length > 0) {
            const { willHeals: cmdheal, ndices } = this._doCmds(pidx, cmds, { heros: aheros, summons: aSummon });
        ...
        this.emit('statusAtk-after', pidx);
        return true;
    }
    /**
    * 检测技能发动
    ...
    */
    _detectSkill(pidx: number, otrigger: Trigger | Trigger[],
        ...
        let { hidxs = allHidxs(heros) } = options;
    ...
    private _detectSlot(pidx: number, otriggers: Trigger | Trigger[], options: {
        ...
        const { players = this.players, summons = players[pidx].summons, heal, hcard, ehidx = players[pidx ^ 1].hidx,
            ...
            isChargedAtk = false, isFallAtk = players[pidx].isFallAtk, isExec = true, isSummon = [], minusDiceSkill = [],
                    ...
                    if (!taskMark) {
                        ...
                        const slotHandle = () => {
                            ...
                            if (!isAddTask || tcmds.length > 0) {
                                ...
                                if (cmdwh != undefined) willHeals = cmdwh;
                            }
     ...
     *                phase 当前最新阶段, players 最新玩家信息, hidxs 只执行某几个角色, heal 回血数,
     ...
     *                isUnshift 是否插入执行, dmgSource 伤害来源id, hasDmg 是否造成伤害, minusDiceSkillIds 减骰id,
     *                isReadySkill 是否为准备技能, source 触发来源id, cStatus 只检测这一个状态, minusDiceSkill 技能当前被x减费后留存的骰子数,
    ...
    */
    private _detectStatus(pidx: number, otypes: StatusType | StatusType[], otrigger: Trigger | Trigger[],
        options: {
            ...
            minusDiceSkill?: number[][], isOnlyExecSts?: boolean, fhidx?: number,
        } = {}) {
        ...
        const { isExec = true, isOnlyFront = false, players = this.players, phase = this.players[pidx].phase, hasDmg = false,
            ...
            isUnshift = false, isSwitchAtk = false, taskMark, skilltype, isSummon = [], dmgSource = -1,
            minusDiceSkillIds = [], isReadySkill = false, source = -1, isChargedAtk = false, isFallAtk = false, cStatus,
            minusDiceSkill = [], isOnlyExecSts = false, dmgedHidx = -1, fhidx } = options;
        let addDiceHero = 0;
        let minusDiceHero = 0;
        let isInvalid = false;
        const cmds: Cmds[] = [];
        ...
        const detectStatus = (stses: Status | Status[], group: StatusGroup, hidx: number, trigger: Trigger) => {
            ...
            for (const sts of stses) {
                ...
                if (isExec && types.includes(STATUS_TYPE.Attack) && !trigger.startsWith('after') && (stsres.damage || stsres.pdmg || stsres.heal)) {
                    (trigger.includes('get') && !trigger.includes('oppo') ? statusAtksPre : statusAtks).push({
                        id: sts.id,
                        entityId: sts.entityId,
                        group,
                        pidx,
                        ...
                        if (stsres.isAddTask) {
                            ...
                            if (!taskMark) {
                                ...
                                const statusHandle = async () => {
                                    const { heros: aheros, hidx: ahidx, summons: asummon, dice } = this.players[pidx];
                                    ...
                                    if (!curStatus()) return;
                                    if (!curStatus().hasType(STATUS_TYPE.TempNonDestroy)) curStatus().type.push(STATUS_TYPE.TempNonDestroy);
                                    ...
                                    this.emit(flag, pidx, { isActionInfo: true, statusSelect: [pidx, group, hidx, curStatusIdx] });
        ...
        for (const trigger of triggers) {
            for (let i = 0; i < pheros.length; ++i) {
                ...
                if ((hidxs ?? [hidx]).includes(hidx) && (chero.isFront || (!isOnlyCombatStatus && !isOnlyFront))) {
                    if (!isOnlyCombatStatus) detectStatus(isCdt(cStatus?.group == STATUS_GROUP.heroStatus, cStatus) ?? chero.heroStatus, STATUS_GROUP.heroStatus, hidx, trigger);
                }
                if (i == 0 && !isOnlyHeroStatus) detectStatus(isCdt(cStatus?.group == STATUS_GROUP.combatStatus, cStatus) ?? player.combatStatus, STATUS_GROUP.combatStatus, fhidx ?? player.hidx, trigger);
            }
        }
        if (isExec && !taskMark) {
        ...
        const { willHeals, supportCnt } = this._doCmds(pidx, cmds, { players, heros: pheros, eheros: peheros, isExec: false })
        return {
            isQuickAction: !!isQuickAction || iqa, addDiceHero, minusDiceHero, switchHeroDiceCnt, statusAtks, isInvalid, minusDiceCard, task,
            addDmg, getDmg, addDmgSummon, nsummons, willHeals, supportCnt, pdmgs, cmds, taskcmds,
        }
    }
    /**
     * 召唤物效果发动
                ...
                if (isExec) {
                    if (!isExecTask) {
                        ...
                        const summonHandle1 = async () => {
                            ...
                            this._writeLog(`[${player.name}]${state}:${smn.name}.useCnt:${oCnt}->${smn.useCnt}`, true);
                            if (smnexecres?.cmds) {
                                const damages = (isOppo: boolean = true, cnt?: number, element?: DamageType, hidxs?: number[]) => {
                                    ...
                                    return {
                                        ...
                                        willDamages: new Array((isOppo ? eHeros : aHeros).length).fill(0).map((_, i) => [
                                            i == eFrontIdx && dmgElement != DAMAGE_TYPE.Pierce ? (cnt || smn.damage || -1) : -1,
                                            (hidxs ?? getBackHidxs(eHeros)).includes(i) ? element == DAMAGE_TYPE.Pierce ? (cnt ?? 0) : smn.pdmg : 0,
                                        ]),
                                    }
                                }
                                const { changedEl, ndices, atkedIdx: [tarHidx] = [-1], willDamages = [], willHeals = [], dmgElements = [], elTips = [] }
                                ...
                                this._doDamage(pidx, {
                                    ...
                                    elTips,
                                    selected,
                                }, { summonSelect: isCdt(willDamages.length == 0 && willHeals.length == 0, selected) });
                                await delay(2500);
    ...
    _doCmds(pidx: number, cmds?: Cmds[],
        ...
        const { withCard, hidxs: chidxs, heal = 0, isExec = true, ahidx = player.hidx, ehidx = opponent.hidx,
            ...
            heroIdx = -1, isSummon = [], isStatus = [], trigger = '' } = options;
        ...
        for (let i = 0; i < cmdlen; ++i) {
            ...
            if (cmd == 'useSkill') {
                    ...
                    while (restCnt-- > 0) {
                        ...
                        if (cards[restCnt]) { // 摸指定卡
                            if (isAttach) { // 从牌库摸
                                ...
                                if (cardIdx > -1 && isExec) [wcard] = player.pile.splice(cardIdx, 1);
                            } else { // 直接生成
                                wcard = cards[restCnt];
                            }
                            ...
                            if (exclude.length > 0) { // 在指定的某几张牌中随机模
                                ...
                                wcard = player.pile.shift() ?? null;
                            }
                        } else { // 指定副类型
                            if (isAttach) {
                                if (player.pile.every(c => !c.hasSubtype(...(subtype as CardSubtype[])) || !c.hasTag(...(cardTag as CardTag[])))) {
                                    break;
                                }
                                while (wcard == null) {
                                    const cardIdx = player.pile.findIndex(c => (c.hasSubtype(...(subtype as CardSubtype[])) || c.hasTag(...(cardTag as CardTag[]))) && !exclude.includes(c.id));
                                    if (cardIdx > -1) {
                                        if (!isExec) break;
                                        [wcard] = player.pile.splice(cardIdx, 1);
                                    }
                                }
                            } else {
                        ...
                        if (wcard && wcard.id != 0 && isExec) {
                            player.UI.willGetCard.push(wcard.setEntityId(this._genEntityId()));
                        }
                    }
                ...
                const { cmds: discmds, summons: dissmns } = this._doDiscard(pidx, discards, { heros: cheros, isAction, isExec });
                cmds.push(...discmds);
                if (dissmns.length > 0) {
                    assgin(player.summons, this._updateSummon(pidx, dissmns, player.summons, player, isExec));
                }
                ...
                ((isOppo ? ceheros : cheros) as Hero[]).forEach((h, hi) => {
                    if (h.hp > 0 && (hidxs == undefined && h.isFront || hidxs?.includes(hi))) {
                        ...
                        this._writeLog(`[${player.name}][${h.name}]获得${cnt}点充能`, true);
                    }
                });
                ...
                (hidxs ?? [(atkOppo ? opponent : player).hidx]).forEach((hidx, hi) => {
                    ...
                    const {
                        willDamages: wdmgs = new Array(players[epidx].heros.length).fill(0).map((_, i) => i == hidx ? [
                            cnt > 0 && (element != DAMAGE_TYPE.Physical && element != DAMAGE_TYPE.Pierce) ? cnt : -1,
                            element == DAMAGE_TYPE.Pierce ? cnt : 0] : [-1, 0]),
                        dmgElement: dmgel = Array.isArray(element) ?
                            element[hi] as ElementType : atkElMode == CMD_MODE.FrontHero ?
                                this._getFrontHero(epidx, { players, offset: cmds[i].hidxs?.[0] }).element :
                                (element ?? DAMAGE_TYPE.Physical) as ElementType,
                    } = damages?.(atkOppo, cnt, element as DamageType | undefined, hidxs) ?? {};
                    ...
                    } = this._calcDamage(pidx, dmgel, wdmgs, hidx, players, { isSummon, isStatus, isAtkSelf: +!atkOppo });
     ...
     * @returns 预览结果
     */
    private _previewSkill(pidx: number, skidx?: number, options: { withCard?: Card, isSwitch?: number } = {}) {
        const { withCard, isSwitch = -1 } = options;
        ...
        for (const sidx of sidxs) {
            ...
            const players = clone(this.players);
            const skill = players[pidx].heros[hidx].skills[sidx];
            if (skill) {
            ...
            const { willHp, willAttachs, summonCnt, supportCnt, willSummons, willSwitch }
                = this._useSkill(pidx, sidx, { isPreview: true, withCard, isSwitch, otriggers: isCdt(sidx == -1, ['change-from', 'change-to']) });
    ...
    private _previewCard(pidx: number) {
        ...
        for (const cidx in handCards) {
            ...
            const player = this.players[pidx];
            // const opponent = players[pidx ^ 1];
            const { isValid: diceValid, diceSelect, skillIdx = -1, switchIdx, heroCanSelect = [] } = this._checkCard(pidx, +cidx);
            ...
            for (const heroSelect of heroSelects) {
                ...
                if (skillIdx > -1) [skillPreview] = this._previewSkill(pidx, skillIdx, { withCard: handCards[cidx], isSwitch: switchIdx });
                const preview: Preview = {
    ...
    _updateStatus(pidx: number, nStatus: Status[], oStatus: Status[], isExec = false, heros?: Hero[], combatStatus?: Status[], hidx: number = -1, ohidx: number = -1) {
        ...
        return oriStatus.sort((a, b) => Math.sign(a.summonId) - Math.sign(b.summonId))
            .filter(sts => {
                ...
                if (isExec && !isNotDestroy) this._doStatusDestroy(pidx, sts);
    ...
    * @param options.readySkillId 准备技能id
