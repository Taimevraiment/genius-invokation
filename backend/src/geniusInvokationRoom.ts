...
export default class GeniusInvokationRoom {
    ...
    private _useSkill(pidx: number, skid: number | SkillType, options: {
            ...
            preview_end: for (const hfield of hfields) {
                ...
                for (const state of ctrgs) {
                    ...
                    if (!isSts || isSmnTrg || isStsRes && (fieldres.damage || fieldres.pdmg || fieldres.heal)) {
                        ...
                        const { isKilled, atriggers, etriggers } = calcAtk(oplayers, fieldres, isSts ? ['hero', 'combat'][group] + 'Status' : 'slot', hfield.id, skid, isSelf);
                        if (isKilled) {
                            isPreviewEnd = true;
                            break preview_end;
                        }
        ...
        if (!isExec) {
            ...
            const atkHidx = isCdt(skillres.skillAfter?.includeCmds('switch'), ohidx);
    ...
    */
    ...
    private _updateStatus(pidx: number, nStatus: Status[], oStatus: Status[], players: Player[],
        ...
        if (nightSoulId > -1) {
            this._detectSlotAndStatus(pidx, 'getNightSoul', {
                types: [STATUS_TYPE.Usage, STATUS_TYPE.Attack],
                players,
                hidxs: allHidxs(heros),
                source: nightSoulId,
