...
export default class GeniusInvokationRoom {
    ...
    isGettingCard: (NodeJS.Timeout | null)[] = [null, null]; // 是否正在抓牌
    isDiscarding: (NodeJS.Timeout | null)[] = [null, null]; // 是否正在弃牌
    isAddingCard: (NodeJS.Timeout | null)[] = [null, null]; // 是否正在加牌入牌库
    ...
    get needWait() {
        return this.taskQueue.isTaskEmpty() &&
            !this.taskQueue.isExecuting &&
            this.isGettingCard.every(t => t == null) &&
            this.isDiscarding.every(t => t == null) &&
            this.isAddingCard.every(t => t == null);
    }
    /**
     * 手动设置种子
     * @param rt 种子
    ...
    emit(flag: string, pidx: number, options: {
        ...
        this.players.forEach(p => {
            ...
            p.heros.forEach(h => {
                if (!notUpdate) assgin(h.heroStatus, this._updateStatus(p.pidx, [], h.heroStatus, true, p.heros, p.combatStatus, h.hidx));
                this._calcSkillChange(p.pidx, h.hidx);
            });
            if (!notUpdate) assgin(p.combatStatus, this._updateStatus(p.pidx, [], p.combatStatus, true, p.heros, p.combatStatus, p.hidx));
            ...
            if (cards.length > 0 && this.isAddingCard[p.pidx] == null) {
                ...
                this.isAddingCard[p.pidx] = setTimeout(() => {
    ...
    private _useSkill(pidx: number, skid: number | SkillType, options: {
        ...
        const { willHeals: skillheal, isSwitch: swc = -1, isSwitchOppo: swco = -1, tasks = [] } = this._doCmds(pidx, skillcmds, {
            players: bPlayers,
            isExec,
            ...
            if ( // 没有元素反应(不含冰草共存)
                ...
                if (dmgElement == ELEMENT_TYPE.Anemo && elTypes.includes(attachElement)) { // 扩散
                    ...
                    if (hasEls(ELEMENT_TYPE.Cryo, ELEMENT_TYPE.Dendro)) { // 冰草共存
                        ...
                        assgin(efhero.heroStatus, this._updateStatus(pidx, [this.newStatus(106)], efhero.heroStatus));
    ...
    private async _useCard(pidx: number, cardIdx: number, diceSelect: boolean[], socket: Socket,
        ...
        if (isInvalid) {
            ...
            const { ndices, phase, willHeals = [], isSwitch = -1, willDamages = [], dmgElements = [], elTips = [] }
    ...
    private _doReset() {
        this.players.forEach(player => {
            ...
            if (rCombatStatus.length > 0) assgin(player.combatStatus, this._updateStatus(player.pidx, rCombatStatus, player.combatStatus, true, player.heros, player.combatStatus, player.hidx));
        });
    }
    /**
     * 执行回合开始阶段检测
        ...
        setTimeout(async () => {
            ...
            this.players.forEach(player => {
                player.heros.forEach((h, hi, a) => {
                    ...
                    assgin(a[hi].heroStatus, this._updateStatus(player.pidx, [], h.heroStatus, true, a, player.combatStatus, hi));
                });
                assgin(player.combatStatus, this._updateStatus(player.pidx, [], player.combatStatus, true, player.heros, player.combatStatus, player.hidx));
    ...
    private _doDamage(pidx: number, damageVO: DamageVO, options: {
        ...
        isQuickAction?: boolean, atkname?: string, supportSelect?: number[],
    } = {}) {
        if (damageVO == -1) return;
        let isDie = -1;
        const { isSwitch = -1, slotSelect, summonSelect, supportSelect, canAction = true, isQuickAction = false, atkname = '' } = options;
        ...
        win: for (const p of this.players) {
            ...
            if (heroDie > -1) {
                if (isSwitch > -1 && this.players[p.pidx].heros[isSwitch].hp > 0) {
                    ...
                    this.isDieBackChange = !isQuickAction && this.players[p.pidx].phase == PHASE.ACTION;
                    this.players[p.pidx].status = PLAYER_STATUS.DIESWITCH;
    ...
    private _doDiscard(pidx: number, discards: Card[], options: {
        ...
        const { players = this.players, isAction = false, isExec = false,
        ...
        for (const card of discards) {
            ...
            if (cardres.cmds) {
                const { willDamages = [], willHeals: [whl] = [], dmgElements = [], elTips = [], atkedIdx: tarHidx = -1 }
                    = this._doCmds(pidx, cardres.cmds, { players, isAction, isExec, supportCnt });
            ...
            for (const trigger of triggers) {
                ...
                if (hstop.length > 0) {
                    (slotres.hidxs ?? [eheros.findIndex(h => h.isFront)]).forEach(hi => {
                        assgin(eheros[hi].heroStatus, this._updateStatus(pidx ^ 1, hstop, eheros[hi].heroStatus));
                    });
                }
                if (cstop.length > 0) {
                    assgin(players[pidx ^ 1].combatStatus, this._updateStatus(pidx ^ 1, cstop, players[pidx ^ 1].combatStatus));
                }
            }
    ...
    */
        ...
        const detectStatus = (stses: Status | Status[], group: StatusGroup, hidx: number, trigger: Trigger) => {
            ...
            for (const sts of stses) {
                ...
                if (isExec || isOnlyExecSts) {
                    ...
                    const stsexecres = stsres.exec?.(undefined, { isExec: isExec && !isOnlyExecSts, switchHeroDiceCnt, isQuickAction: isTriggeredQuick });
    ...
    _doCmds(pidx: number, cmds?: Cmds[],
        ...
        const { withCard, hidxs: chidxs, heal = 0, isExec = true, ahidx = player.hidx, ehidx = opponent.hidx,
            heros: ocheros, eheros: oceheros, isAction = false, source = -1, damages, socket,
            isSummon = -1, trigger = '', getdmg = [], willSwitch = players.map(p => p.heros.map(() => false)),
            supportCnt = players.map(() => new Array(MAX_SUPPORT_COUNT).fill(0)) } = options;
            ...
            if (cmd == 'useSkill') {
                ...
                if (cmd == 'getCard') {
                    ...
                    if (willGetCard.length > 0 && isExec) {
                        cplayer.UI.willGetCard.push(...willGetCard);
                        // this.taskQueue.addTask('getCard', [[async () => {
                        //     const p = this.players[cpidx];
                        //     p.UI.willGetCard.push(...willGetCard);
                        //     this._writeLog(`[${p.name}]抓${willGetCard.length}张牌【p${cpidx}:${willGetCard.map(c => `[${c.name}]`).join('')}】`);
                        //     this.emit('doCmd--getCard', cpidx);
                        //     await delay(1500);
                        //     const rest = MAX_HANDCARDS_COUNT - p.handCards.length;
                        //     p.handCards.push(...p.UI.willGetCard.slice(0, rest));
                        //     p.UI.willGetCard = [];
                        //     this.emit('doCmd--getCard-after', cpidx);
                        // }, 300]]);
                    }
                }
                if (cmd == 'addCard' && isExec) {
                    cplayer.UI.willAddCard.cards.push(...cards);
                    cplayer.UI.willAddCard.scope = cmds[i].hidxs?.[0] ?? 0;
                    cplayer.UI.willAddCard.isRandom = !isAttach;
                    cplayer.UI.willAddCard.isNotPublic = mode == CMD_MODE.isNotPublic;
                    // this.taskQueue.addTask('addCard', [[async () => {
                    //     const p = this.players[cpidx];
                    //     // todo willAddCard不用这么多属性了
                ...
                if (isExec) {
                    ...
                    if (mode == CMD_MODE.Random || mode == CMD_MODE.AllHandCards || mode == CMD_MODE.HighHandCard || card) { // 舍弃手牌
                        discardIdxs.forEach(dcidx => player.UI.willDiscard[0].push(player.handCards[dcidx]));
        ...
        if (willDamages != undefined && dmgElement != undefined) {
            const { players: players1, willDamages: willDamage1, elTips: elTips1, dmgElements: dmgElements1, willHeal: willHeal1 }
        ...
        for (const sid of skids) {
            ...
            const { willHp, willAttachs, summonCnt, supportCnt, willSummons, willSwitch }
                = this._useSkill(pidx, sid, {
                    ...
                    isSwitch,
                    willDamages,
                    dmgElement, otriggers: isCdt(sid == -1, ['change-from', 'change-to']),
                    atkedIdx,
                });
        ...
        newSummon.forEach(smn => {
            ...
            if (csmnIdx > -1) { // 重复生成召唤物
                ...
                if (smnres.rCombatStatus) {
                    const nsts = this._updateStatus(pidx, this._getStatusById(smnres.rCombatStatus), combatStatus, isExec, heros, combatStatus, hidx);
