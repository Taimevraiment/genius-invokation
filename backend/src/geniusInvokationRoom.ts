...
export default class GeniusInvokationRoom {
    ...
    private _useSkill(pidx: number, skidx: number, options: { isPreview?: boolean, withCard?: Card, isSwitch?: number, isReadySkill?: boolean, otriggers?: Trigger | Trigger[] } = {}) {
        ...
        if (skidx > -1) {
            ...
            const { willHeals: skillheal } = this._doCmds(pidx, skillres.cmds, { players, isExec });
        ...
        if (!isExec) {
            ...
            const { isDie } = this._doDamage(pidx, {
                ...
                tarHidx: aWillDamages.some(([d, p]) => d > -1 || p > 0) ? dmgedHidx : -1,
                willDamages: aWillDamages,
                dmgElements,
                elTips: aElTips,
                willHeals: aWillHeal,
            }, { isSwitch });
            if (isSwitch == -1) this._changeTurn(pidx, isQuickAction, 'useSkill', { isDie });
        }
    ...
    private async _useCard(pidx: number, cardIdx: number, diceSelect: boolean[], socket: Socket,
        ...
        if (isInvalid) {
            ...
            if (isUseSkill) {
                await delay(2450);
                await wait(() => !this.taskQueue.isExecuting, { delay: 0 });
            }
            await this._execTask();
        }
    ...
    */
    ...
    private _detectSlot(pidx: number, otriggers: Trigger | Trigger[], options: {
        ...
        for (const hidx of hidxs) {
            ...
            if (hcard && slots.every(slot => slot?.id != hcard.id)) slots.push(hcard);
            for (const slot of slots) {
                ...
                if (isExec && (tcmds.length > 0 || isAddTask)) {
                    if (!taskMark) {
                        ...
                        if (tcmds.some(({ cmd }) => cmd == 'heal')) intvl += 2000;
                        const slotHandle = () => {
    ...
    */
    ...
    _doCmds(pidx: number, cmds?: Cmds[],
        ...
        for (let i = 0; i < cmdlen; ++i) {
            ...
            if (cmd == 'useSkill') {
                ...
                if (nhidx > -1 && !hasObjById(heros.find(h => h.isFront)?.heroStatus, 300005)) {
                    cmds[i].hidxs = [nhidx];
    ...
    private _previewSkill(pidx: number, skidx?: number, withCard?: Card, isSwitch: number = -1) {
        ...
        const previews: Preview[] = [];
        const { skills, energy, maxEnergy, heroStatus, hidx } = isSwitch != -1 ? this.players[pidx].heros[isSwitch!] : this._getFrontHero(pidx);
