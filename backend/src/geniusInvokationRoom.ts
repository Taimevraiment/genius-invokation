...
export default class GeniusInvokationRoom {
    ...
    private _useSkill(pidx: number, skid: number | SkillType, options: {
        ...
        const doPreviewHfield = (
            ...
            if (group == STATUS_GROUP.heroStatus) {
                ...
                this._detectSlotAndStatus(cpidx, afterAndSwitchTrgs, { types: STATUS_TYPE.Usage, players: oplayers, energyCnt, hidxs: chidx, isExec, atkHidx });
            }
        ...
        if (!isExec) {
            ...
            willSwitch[pidx].filter(v => v).forEach(() => {
                const triggers: Trigger[] = skid == -1 && atkHidx == undefined ? dtriggers : ['switch-from', 'switch-to', 'switch'];
                if (!isPreviewEnd) ({ isPreviewEnd } = doPreviewHfield(bPlayers, this._getHeroField(pidx, { players: bPlayers, hidx: ahidx(), isOnlyHeroStatus: true }), ahidx(), STATUS_GROUP.heroStatus, triggers, isExec, 1, false, atkHidx));
                if (!isPreviewEnd) ({ isPreviewEnd } = doPreviewHfield(bPlayers, bPlayers[pidx].combatStatus, ahidx(), STATUS_GROUP.combatStatus, triggers, isExec, 1, false, atkHidx));
            });
            players.forEach(p => {
    ...
    private _checkCard(pidx: number, players: Player[], cardIdx: number, options: { isReconcile?: boolean, heroIdx?: number, summonIdx?: number } = {}) {
        ...
        if (!currCard) throw new Error(`@_checkCard:p${pidx}的卡牌${cardIdx}不存在,手牌:${handCards.map(c => `[${c.name}]`)}`);
        ...
        for (const card of discards) {
            ...
            if (cardres.cmds) {
                const { bWillDamages = [] } = this._doCmds(pidx, cardres.cmds,
                    { players: isCdt(!isExec, clone(players)), isAction, isExec, supportCnt, isPriority, isUnshift });
                    ...
                    if (isExec) {
                        this.taskQueue.addTask(`doDamage-${atkname}`, [[async () => {
                            const { bWillDamages: willDamages = [], willHeals: [whl] = [], bDmgElements: dmgElements = [],
                                ...
                                = this._doCmds(pidx, cardres.cmds?.filterCmds('attack'), { isAction });
                            await this._doDamage(pidx, {
                                dmgSource: 'card',
                                atkPidx: pidx,
                                atkHidx: -1,
    ...
    private _doEquip(pidx: number, hero: Hero, equipment: Card | number, options: { isDestroy?: boolean, isExec?: boolean } = {}) {
        ...
        if (!isDestroy) {
            ...
            if (equipment.hasTag(CARD_TAG.Enchant)) {
                ...
                if (!attachEl) throw new Error('@_doEquip: Enchant equipment must have attachEl');
    ...
    */
    private _detectSkill(pidx: number, otrigger: Trigger | Trigger[],
        ...
        return { isQuickAction, task, restDmg, dmgElement, skillPreviews, tasks }
    }
    /**
     * 检测装备发动
     * @param pidx 玩家序号
    ...
    */
    private _detectStatus(pidx: number, otypes: StatusType | StatusType[], otrigger: Trigger | Trigger[], options: {
        ...
        orderAfter?: string, isSwirlExec?: boolean, atkHidx?: number,
    } = {}) {
        ...
        const { isExec = true, isOnlyFront, players = this.players, hasDmg, orderAfter, isSwirlExec,
            ...
            willSwitch = players.map(p => p.heros.map(() => false)), isPriority, energyCnt, atkHidx } = options;
        let isInvalid = false;
        const cmds = new CmdsGenerator();
    ...
    private _detectSlotAndStatus(pidx: number, otrigger: Trigger | Trigger[], options: {
        ...
        isSwirlExec?: boolean, atkHidx?: number,
    }) {
        const triggers = convertToArray(otrigger);
    ...
    private _doCmds(pidx: number, cmds?: Cmds[] | CmdsGenerator,
        ...
        for (let i = 0; i < cmds.length; ++i) {
            ...
            if (cmd == 'useSkill') {
                ...
                this._detectSkill(pidx, 'consumeNightSoul', { players, sourceHidx: ohidxs[0], source: nightSoul?.id, energyCnt, isExec });
                const { tasks: nstasks = [] } = this._detectSlotAndStatus(pidx, 'consumeNightSoul', { players, hidxs: allHidxs(player.heros), sourceHidx: ohidxs[0], energyCnt, isExec });
                tasks.push(...nstasks);
        ...
        for (let fhidx = 0; fhidx < cheros.length; ++fhidx) {
            ...
            if (heroStatus && heroStatus[fhidx].length) {
                const { tasks: ststasks = [] } = this._updateStatus(pidx, heroStatus[fhidx], fhero.heroStatus, players,
                    { hidx: fhidx, isExec, supportCnt, energyCnt, isQuickAction: !isAction });
                tasks.push(...ststasks);
            }
        }
        if (combatStatus) {
            this._updateStatus(pidx, combatStatus, player.combatStatus, players,
                { hidx: ahidx, isExec, supportCnt, energyCnt, isQuickAction: !isAction });
        }
        ...
        if (combatStatusOppo) {
            this._updateStatus(pidx ^ 1, combatStatusOppo, opponent.combatStatus, players,
                { hidx: ehidx, isExec, supportCnt, energyCnt, isQuickAction: !isAction });
        }
        return {
    ...
    private _updateStatus(pidx: number, nStatus: Status[], oStatus: Status[], players: Player[],
        options: {
            ...
            isLog?: boolean, isAddAtkStsTask?: boolean, energyCnt?: number[][],
        } = {}
    ) {
        const newStatus: Status[] = clone(nStatus);
        const player = players[pidx];
        const tasks: AtkTask[] = [];
        if (!player) return { tasks }
        const { isExec, ohidx, supportCnt, isQuickAction, isLog = true, isAddAtkStsTask, energyCnt } = options;
        let { hidx } = options;
        ...
        newStatus.forEach(sts => {
            ...
            if (cstIdx > -1) { // 重复生成状态
                ...
                if (sts.maxCnt == 0) { // 不可叠加
                    ...
                    let oCnt = -1;
                    let nCnt = -1;
                    if (cStatus.hasType(STATUS_TYPE.NightSoul)) nightSoulId = cStatus.id;
                ...
                oStatus.push(sts);
                if (sts.hasType(STATUS_TYPE.NightSoul)) nightSoulId = sts.id;
