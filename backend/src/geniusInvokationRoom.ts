...
export default class GeniusInvokationRoom {
    ...
    private _useSkill(pidx: number, skid: number | SkillType, options: {
        ...
        // 判断准备技能和切换角色的技能
        const skill = this._calcSkillChange(pidx, hidx, { isReadySkill, isSwitch, skid, players, isExec });
        ...
        const nonAction = !isExec && player().heros[hidx].heroStatus.some(s => s.hasType(STATUS_TYPE.NonAction));
        ...
        const calcAtk = (oplayers: Player[], res: CalcAtkRes, type: string, stsId: number, skid = -1, isSelf = 0) => {
            ...
            if (isSwitchOppo(elrcmds3[0])) atriggers3[cahidx].push('switch-from', 'switch-to');
            ...
            if (isSwitchOppo(elrcmds3[0])) etriggers3[cehidx].push('switch-from', 'switch-to');
        ...
        if (isSwitch > -1 && !isExec) { // 切换角色后的协同攻击预览
            doPreviewStatus(players, players[pidx].heros[ahidx()].heroStatus, ahidx(), STATUS_GROUP.heroStatus, ['switch-from'], false, 1);
            doPreviewStatus(players, players[pidx].heros[isSwitch].heroStatus, isSwitch, STATUS_GROUP.heroStatus, ['switch-to'], false, 1);
            doPreviewStatus(players, players[pidx].combatStatus, isSwitch, STATUS_GROUP.combatStatus, ['switch-from', 'switch-to'], false, 1);
        }
        ...
        if (isSwitchOppo(skillcmds)) {
            doPreviewStatus(bPlayers, bPlayers[epidx].heros[odmgedHidx].heroStatus, odmgedHidx, STATUS_GROUP.heroStatus, ['switch-from', 'switch-to'], isExec);
            doPreviewStatus(bPlayers, bPlayers[epidx].combatStatus, odmgedHidx, STATUS_GROUP.combatStatus, ['switch-from', 'switch-to'], isExec);
        }
            ...
            if ( // 没有元素反应(不含冰草共存)
                ...
                if (dmgElement == ELEMENT_TYPE.Anemo && elTypes.includes(attachElement)) { // 扩散
                    ...
                    if (hasEls(ELEMENT_TYPE.Cryo, ELEMENT_TYPE.Dendro)) { // 冰草共存
                        ...
                        if (efhero.isFront) {
                            const cmds: Cmds[] = [{ cmd: 'switch-after', isOppo: !isAtkSelf }];
                            this._doCmds(pidx, cmds, {
                                players: res.players,
                                isPriority: true,
                                isExec,
        ...
        if (!isAttach) {
            ...
            for (let i = 0; i < ehlen; ++i) {
                ...
                const hfieldres = this._detectSlotAndStatus(dmgedPidx, trgs, {
                    ...
                    discards,
                    hcard: withCard,
                    isUnshift: chi == dmgedHidx && !isSwirl && trgs.some(trg => trg.includes('get') && !trg.includes('oppo')),
                });
    ...
    private async _useCard(pidx: number, cardIdx: number, diceSelect: boolean[], socket: Socket,
        ...
        // await this._execTask();
        // const { isInvalid, minusDiceCard: stsmdc, isQuickAction: iqa, isFallAtk: ifa } = this._detectStatus(pidx, [STATUS_TYPE.Attack, STATUS_TYPE.Usage], 'card', { hcard: currCard, isOnlyFront: true, minusDiceCard });
        // await this._execTask();
        // this._detectStatus(pidx, STATUS_TYPE.Usage, 'card', { hcard: currCard, hidxs: getBackHidxs(player.heros), isOnlyHeroStatus: true, minusDiceCard });
        // await this._execTask();
        // this._detectSkill(pidx, 'card');
        await this._execTask();
    ...
    private async _doDamage(pidx: number, damageVO: DamageVO, options: {
        ...
        const { slotSelect, summonSelect, supportSelect, canAction = true, isQuickAction = false, atkname = '', cmds = [], skid = -1 } = options;
        ...
        if (isDie.size > 0) {
            ...
            for (const cpidx of isDie) {
                if (cpidx != pidx) {
                    ...
                    // this._detectStatus(pidx, STATUS_TYPE.Usage, 'kill', { isOnlyFront: true, isUnshift: true });
                }
            }
            this.taskQueue.addTask(`heroDie-${damageVO.dmgSource}-${atkname}`, [[() => {
                ...
                for (const cpidx of isDie) {
                    ...
                    for (const chidx of heroDie[cpidx]) {
                        ...
                        if (!cplayer.heros[chidx].isFront) {
    ...
    _detectHeal(pidx: number, heal: number[], players: Player[], options: {
        ...
        if (heal.some(hl => hl > -1)) {
            ...
            this._detectSlotAndStatus(pidx, 'heal', { types: STATUS_TYPE.Usage, hidxs: allHidxs(heros), players, heal, isQuickAction, supportCnt, isExec });
    ...
    */
    private _detectStatus(pidx: number, otypes: StatusType | StatusType[], otrigger: Trigger | Trigger[], options: {
        ...
        players?: Player[], hidxs?: number | number[], skilltype?: SkillType, hcard?: Card, discards?: Card[], getdmg?: number[], slotsDestroy?: number[],
    ...
    _doCmds(pidx: number, cmds?: Cmds[],
        options: {
            ...
            damages?: SmnDamageHandle, isOnlyGetWillDamages?: boolean, isPriority?: boolean,
        } = {}
    ): {
        ...
        const { withCard, hidxs: chidxs, heal = 0, isExec = true, ahidx = player.hidx, ehidx = opponent.hidx,
            ...
            supportCnt = players.map(() => new Array(MAX_SUPPORT_COUNT).fill(0)), isPriority } = options;
                ...
                if (nhidx > -1) {
                    ...
                    if (!isExec) {
                        ...
                        if (!cmds.some(({ cmd }) => cmd == 'useSkill') && !isOppo) {
                            this._useSkill(pidx, -1, { isPreview: true, isSwitch, otriggers: ['switch-from', 'switch-to'] });
                        }
                    } else {
