...
export default class GeniusInvokationRoom {
    ...
    private _useSkill(pidx: number, skid: number | SkillType, options: {
        ...
        if (!isExec) {
            if (isSwitchOppo(skillcmds) || etriggers[odmgedHidx].includes('get-Overload')) {
    ...
    private _checkCard(pidx: number, players: Player[], cardIdx: number, options: { isReconcile?: boolean, heroIdx?: number, summonIdx?: number } = {}) {
        ...
        if (!cardres.notPreview && !notSelectHero) {
    ...
    private _detectHeal(pidx: number, heal: number[], players: Player[], options: {
        ...
        if (heal.some(hl => hl > -1)) {
            if (!notPreHeal) this._detectStatus(pidx, STATUS_TYPE.Usage, 'pre-heal', { heal, players, hidxs: allHidxs(heros), supportCnt, source, isQuickAction, isExec, isOnlyExecSts: !isExec });
    ...
    private _doDiscard(pidx: number, discards: Card[], options: {
        ...
        const { bWillHeals: stsheal, } = this._detectStatus(pidx, STATUS_TYPE.Usage, 'discard', {
            discards, players, isExec, isOnlyExecSts: !isExec, isQuickAction: !isAction, supportCnt,
        });
    ...
    */
    ...
    private _detectSlot(pidx: number, otriggers: Trigger | Trigger[], options: {
        ...
        hcardsCnt?: number, energyCnt?: number[][], source?: number, dmgSource?: number,
    } = {}) {
        const triggers: Trigger[] = [];
                ...
                if (!taskMark) {
                    ...
                    const slotHandle = async () => {
                        ...
                        if (isDestroy) {
                            ...
                            destroySlot();
                            this.emit(`_doSlot-${slot.name}-destroy`, pidx);
                        }
                    };
    ...
    */
    private _detectStatus(pidx: number, otypes: StatusType | StatusType[], otrigger: Trigger | Trigger[], options: {
        ...
        energyCnt?: number[][], dmg?: number[], sourceHidx?: number,
    } = {}) {
        const types: StatusType[] = [];
        const triggers: Trigger[] = [];
        ...
        const { isExec = true, isOnlyFront, players = this.players, hasDmg,
            ...
            isChargedAtk, isFallAtk, cStatus, minusDiceSkill = [], isOnlyExecSts, dmgedHidx = -1,
    ...
    private _detectSupport(pidx: number, ostates: Trigger | Trigger[],
        ...
        const { hcard, players = this.players, isExec = true, firstPlayer = -1, hidx = -1, skid = -1, sktype,
            isExecTask, csupport, getdmg, heal, discard = 0, minusDiceSkillIds = [], minusDiceSkill = [],
    ...
    private _previewSwitch(pidx: number, switchHidx?: number) {
        ...
        for (const hidx of hidxs) {
            this._random = curRandom;
            const players = clone(this.players);
            const { switchHeroDiceCnt, isQuickAction } = this._calcHeroSwitch(pidx, hidx, player.hidx);
    ...
    */
    ...
    private _calcHeroSwitch(pidx: number, tohidx: number, fromhidx: number, isExec = false) {
        ...
        ({ switchHeroDiceCnt, isQuickAction } = this._detectSlotAndStatus(pidx, 'minus-switch-to', { isExec, hidxs: tohidx, switchHeroDiceCnt, isQuickAction }));
        ...
        return { switchHeroDiceCnt, isQuickAction }
        // let minusDiceHero = 0;
        // let addDiceHero = 0;
        // const { minusDiceHero: stsmh, addDiceHero: stsah } = this._detectSlotAndStatus(pidx, 'active-switch-from', { types: STATUS_TYPE.Usage, isExec: false, hidxs: fromhidx });
        // addDiceHero += stsah;
        // addDiceHero += this._detectSummon(pidx ^ 1, 'active-switch-oppo', { isUnshift: true, isExec: false }).addDiceHero;
        // if (isExec) return INIT_SWITCH_HERO_DICE + addDiceHero;
        // minusDiceHero += stsmh;
        // minusDiceHero += this._detectSlot(pidx, 'active-switch-to', { isExec: false, hidxs: tohidx }).minusDiceHero;
        // minusDiceHero += this._detectSummon(pidx, 'active-switch', { isExec: false }).minusDiceHero;
