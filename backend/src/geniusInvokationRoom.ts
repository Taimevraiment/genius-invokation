...
export default class GeniusInvokationRoom {
    ...
    private _useSkill(pidx: number, skid: number | SkillType, options: {
        ...
        for (let i = 0; i < ahlen; ++i) {
            ...
            const { atkStatus: atkhst, isPreviewEnd } = doPreviewHfield(bPlayers, this._getHeroField(pidx, { players: bPlayers, hidx: hi, isOnlyHeroStatus: true }), hi, STATUS_GROUP.heroStatus, afterASkillTrgs[hi], isExec, 1);
    ...
    private _calcDamage(
        ...
        this._updateSummon(atkPidx, [], res.players, isExec, { destroy: +!isExec });
        if (!isSwirl && isExec) {
    ...
    private async _doDamage(pidx: number, damageVO: DamageVO, options: {
        ...
        heroSelect?: number[], summonDestroy?: number, isActionInfo?: boolean,
    } = {}) {
        ...
        const { slotSelect, summonSelect, supportSelect, heroSelect, canAction = true, isQuickAction = false,
            atkname = '', cmds = [], skid = -1, summonDestroy, isActionInfo = true } = options;
    ...
    */
     ...
     *                        hcardsCnt 当前手牌数, energyCnt 充能变化, source 触发来源id, smgSource 造成伤害来源id
    ...
    private _detectSlot(pidx: number, otriggers: Trigger | Trigger[], options: {
                    ...
                    if (!taskMark) {
                        ...
                        const slotHandle = async () => {
                            ...
                            if (isDestroy) {
                                ...
                                destroySlot();
                                this.emit(`_doSlot-${slot.name}(${slot.entityId})-destroy`, pidx);
                            }
                        };
        ...
        return {
            ...
            supportCnt, heroStatus, combatStatus, nsummons, isQuickAction, minusDiceCard, cmds, task,
        }
    }
    /**
     * 状态效果发动
    ...
    */
    private _detectStatus(pidx: number, otypes: StatusType | StatusType[], otrigger: Trigger | Trigger[], options: {
        ...
        let { isQuickAction = false, switchHeroDiceCnt = 0, minusDiceCard = 0 } = options;
                                ...
                                if (stsres.isAddTask) {
                                    ...
                                    if (!taskMark) {
                                        ...
                                        const statusHandle = async () => {
                                            ...
                                            if (!curStatus.hasType(STATUS_TYPE.Hide)) this._writeLog(`[${this.players[pidx].name}][${curStatus.name}]发动${oCnt != sts.useCnt ? ` ${useCntChange}` : ''}`, stsres?.notInfo ? 'system' : 'info');
    ...
    private _detectSlotAndStatus(pidx: number, otrigger: Trigger | Trigger[], options: {
        ...
        hcardsCnt?: number, energyCnt?: number[][], sourceHidx?: number, isOnlyHeroStatus?: boolean,
    }) {
        const triggers: Trigger[] = [];
        if (typeof otrigger == 'string') triggers.push(otrigger);
        else triggers.push(...otrigger);
        ...
        let { types = [], hidxs = [ahidx], switchHeroDiceCnt } = options;
        if (Array.isArray(hidxs)) {
            hidxs = hidxs.map(hi => (hi - ahidx + heros.length) % heros.length).sort().map(hi => (hi + ahidx) % heros.length);
        } else if (hidxs < 0) {
            hidxs = [];
            ...
            for (const field of heroField) {
                if ('group' in field) {
                    ...
                    const { switchHeroDiceCnt: slotSwitchHeroDiceCnt, addDmg: slotAddDmg, isQuickAction: slotIqa,
                        heroStatus: slotHeroStatus, willHeals: slotWhl, combatStatus: slotCombatSatatus,
        ...
        return {
            minusDiceCard, switchHeroDiceCnt, isInvalid, cmds, aWillHeals, bWillHeals, isQuickAction,
    ...
    private _doCmds(pidx: number, cmds?: Cmds[],
        ...
        for (let i = 0; i < cmdLen; ++i) {
            ...
            if (cmd == 'useSkill') {
                ...
                const discardIdxs = (ohidxs ?? []).map(ci => unselectedCards[ci].entityId);
                        ...
                        if (mode == CMD_MODE.AllHandCards) { // 弃置所有手牌
                            ...
                            while (discardCnt > 0) {
                                if (mode == CMD_MODE.Random) { // 弃置随机手牌
                                    ...
                                    const [{ cidx: cdidx }] = isDiscard ? this._randomInArr(maxCostCards) : maxCostCards;
                    ...
                    if (isExec) {
                        ...
                        this.taskQueue.addTask(`doCmd--discard-p${cpidx}:${cardNames}`, [[async () => {
                            ...
                            if (isDiscardHand) { // 舍弃手牌
                                discards.forEach(dc => p.UI.willDiscard[0].push(dc));
                            ...
                            this._writeLog(`[${p.name}]${isDiscard ? '舍弃了' : '被夺取了'}${cardNames}`);
