...
export default class GeniusInvokationRoom {
    ...
    emit(flag: string, pidx: number, options: {
        ...
        this.players.forEach(p => {
            ...
            p.heros.forEach(h => {
                if (!notUpdate) this._updateStatus(p.pidx, [], h.heroStatus, this.players, h.hidx, true);
                this._calcSkillChange(p.pidx, h.hidx);
            });
            if (!notUpdate) this._updateStatus(p.pidx, [], p.combatStatus, this.players, p.hidx, true, isCdt(p.pidx == pidx, ohidx));
    ...
    private _rollDice(pidx: number, diceSelect: boolean[] = []) {
        ...
        const weight = ([el, cnt]: [DiceCostType, number]) => {
            return DICE_WEIGHT.indexOf(el) - +(el == DICE_COST_TYPE.Omni) * 201 - +heroEle.includes(el) * 200 - cnt * 10;
        }
    ...
    private _selectDice(player: Player, costType: CostType, elDiceCnt: number, anyDiceCnt: number) {
        ...
        const heroEle = player.heros.filter(h => h.element != ELEMENT_TYPE.Physical).map(h => h.element) as DiceCostType[];
    ...
    private _useSkill(pidx: number, skid: number | SkillType, options: {
        ...
        selectSummon?: number, supportCnt?: number[][],
    } = {}) {
        ...
        const summonCnt: number[][] = cplayers.map(() => new Array(MAX_SUMMON_COUNT).fill(0))
        const willSummons: Summon[][] = [[], []];
        if (skill && skill.canSelectSummon != -1 && selectSummon == -1) {
            if (isExec) this.emit('useSkillSelectSummonInvalid', pidx, { tip: '未选择召唤物' });
            return {
                willHp, willAttachs: bWillAttach, elTips: aElTips, dmgElements: aDmgElements, willHeal: aWillHeal,
                players, summonCnt, supportCnt, willSummons, willSwitch, willDamages: aWillDamages, elrcmds: [[], []] as Cmds[][],
            }
        }
        ...
        if (skill && isExec) {
            ...
            }, tarHidx == -1 && aWillDamages.every(([d, p]) => d == -1 && p == 0) && aWillHeal.every(h => h == -1) ? 1e3 : 2250]], true);
        }
        ...
        for (const smn of (isExec ? players : bPlayers)[pidx].summons) {
            ...
            strigger.forEach(trg => {
                const trounds = [0];
                const { smnres } = this._detectSummon(pidx, trg, { csummon: [smn], heros: isCdt(!isExec, bPlayers[pidx].heros), hcard: withCard, isExec });
                while (trounds.length && !isExec) {
                    ...
                    if (smnres) {
                        const element = smn.element;
                        let damage: number | undefined;
                        let pdmg: number | undefined;
                        let hidxs: number[] | undefined;
                        cmds?.filter(c => c.cmd == 'attack').forEach(c => {
                            if (damage == undefined) damage = 0;
                            if (c.element != DAMAGE_TYPE.Pierce) {
                                damage += c.cnt ?? smn.damage;
                            } else {
                                if (pdmg == undefined) pdmg = 0;
                                pdmg += c.cnt ?? smn.pdmg;
                                hidxs = c.hidxs;
                            }
                        });
                        calcAtk(bPlayers, { ...smnres, damage, element, pdmg, hidxs }, 'summon', smn.id, skid, 1);
        ...
        return {
            willHp, willAttachs: bWillAttach, elTips: aElTips, dmgElements: aDmgElements, willHeal: aWillHeal,
            players, summonCnt, supportCnt, willSummons, willSwitch, willDamages: aWillDamages, elrcmds: [skillcmds, eskillcmds],
        }
    }
    /**
     * 计算伤害
    ...
    private _calcDamage(
            ...
            if ( // 没有元素反应(不含冰草共存)
                ...
                if (dmgElement == ELEMENT_TYPE.Anemo && elTypes.includes(attachElement)) { // 扩散
                    ...
                    if (hasEls(ELEMENT_TYPE.Cryo, ELEMENT_TYPE.Dendro)) { // 冰草共存
                        ...
                        aist[dmgedPidx].push(this.newStatus(106));
        ...
        this._updateSummon(dmgedPidx, [], res.players, isExec, { destroy: true });
        this._updateSummon(pidx, [], res.players, isExec);
        if (!isSwirl && isExec) {
    ...
    private _checkCard(pidx: number, players: Player[], cardIdx: number, options: { isReconcile?: boolean, heroIdx?: number } = {}): {
        ...
        if (isReconcile) [ncost, anydice, costChange, costType] = [1, 0, 0, DICE_TYPE.Any];
    ...
    private _doReset() {
        this.players.forEach(player => {
            ...
            player.heros.forEach(h => { // 重置技能
                h.skills.forEach(skill => skill.handle({ reset: true, skill, hero: h, skid: skill.id }))
            });
            ...
            if (rCombatStatus.length > 0) this._updateStatus(player.pidx, rCombatStatus, player.combatStatus, this.players, player.hidx, true);
        });
    }
    /**
     * 执行回合开始阶段检测
        ...
        setTimeout(async () => {
            ...
            this.players.forEach(player => {
                player.heros.forEach(h => {
                    ...
                    this._updateStatus(player.pidx, [], h.heroStatus, this.players, h.hidx, true);
                });
                this._updateStatus(player.pidx, [], player.combatStatus, this.players, player.hidx, true);
            ...
            for (const trigger of triggers) {
                ...
                if (hstop.length > 0) {
                    (slotres.hidxs ?? [eheros.findIndex(h => h.isFront)]).forEach(hi => {
                        this._updateStatus(pidx ^ 1, hstop, eheros[hi].heroStatus, players, hi);
                    });
                }
                if (cstop.length > 0) {
    ...
    */
            ...
            for (const sts of stses) {
                ...
                const stsres = sts.handle(sts, {
                    ...
                    talent: isExec ? getObjById(pheros, getHidById(sts.id))?.talentSlot :
                        isCdt(card?.id == getTalentIdByHid(getHidById(sts.id)), card) ??
                    ...
                    if (!isExecTask) {
                        ...
                        const summonHandle1 = async () => {
                            ...
                            if (smnexecres?.cmds) {
                                ...
                                const isNotAtk = willDamages.length == 0 && (willHeals?.length ?? 0) == 0;
                                this._doDamage(pidx, {
                                    dmgSource: 'summon',
                                    atkPidx: pidx,
                                    atkHidx: -1,
                                    ...
                                    elTips,
                                    selected,
                                }, { atkname: smn.name, summonSelect: isCdt(isNotAtk, selected) });
                                await delay(isNotAtk ? 1100 : 2200);
    ...
    _doCmds(pidx: number, cmds?: Cmds[],
        ...
        for (let i = 0; i < cmdLen; ++i) {
            ...
            if (cmd == 'useSkill') {
                ...
                if (isSwitch == -1) isSwitch = cmds[i].hidxs?.[0] ?? -1;
                let nhidx = -1;
                let sdir = 0;
                ...
                if (nhidx > -1) {
                    ...
                    if (!isExec) {
                        ...
                        this.taskQueue.addTask(`doCmd--switch:${source}`, [[() => {
                            const nhidx = this._getFrontHero(cpidx, { offset: sdir }).hidx;
                            if (hasObjById(this.players[cpidx].heros.find(h => h.isFront)?.heroStatus, 300005)) return;
                            this._switchHero(cpidx, nhidx, `doCmd--switch:${source}`, { socket, trigger })
                        }, 0, cnt]]);
                    }
                }
        ...
        if (willDamages != undefined && dmgElement != undefined) {
            const { players: players1, willDamages: willDamage1, elTips: elTips1, dmgElements: dmgElements1, willHeal: willHeal1, elrcmds: elrcmds1 }
        ...
        for (let fhidx = 0; fhidx < cheros.length; ++fhidx) {
            ...
            if (heroStatus && heroStatus[fhidx].length) {
                heroStatus[fhidx] = heroStatus[fhidx].filter(sts => !sts.hasType(STATUS_TYPE.NonAction));
                this._updateStatus(pidx, heroStatus[fhidx], fhero.heroStatus, players, fhidx, true);
            }
        }
        if (combatStatus) this._updateStatus(pidx, combatStatus, player.combatStatus, players, ahidx, true);
        for (let fhidx = 0; fhidx < ceheros.length; ++fhidx) {
            const fhero = ceheros[fhidx];
            if (heroStatusOppo && heroStatusOppo[fhidx].length) {
                heroStatusOppo[fhidx] = heroStatusOppo[fhidx].filter(sts => !sts.hasType(STATUS_TYPE.NonAction));
                this._updateStatus(pidx ^ 1, heroStatusOppo[fhidx], fhero.heroStatus, players, fhidx, true);
            }
        }
        if (combatStatusOppo) this._updateStatus(pidx ^ 1, combatStatusOppo, opponent.combatStatus, players, ehidx, true);
        return {
    ...
    private _previewSkill(pidx: number, skid?: number, options: {
        ...
        for (const skillId of skids) {
            ...
            if (skill) {
                ...
                const skillres = skill.handle({ skill, hero, skid: skillId });
            ...
            for (const selectSummon of summonSelects) {
                ...
                const preview: Preview = {
                    ...
                    skillId,
                    diceSelect: this._checkSkill(pidx, skillId, skillId == -1 || skill?.isForbidden, isSwitch),
                    willHp,
                    willAttachs,
     ...
     * @returns 合并后状态
     */
    _updateStatus(pidx: number, nStatus: Status[], oStatus: Status[], players: Player[], hidx?: number, isExec = false, ohidx?: number) {
    ...
    private _updateSummon(
        ...
        const { summons, combatStatus, hidx } = players[pidx];
        ...
        assgin(summons, oriSummon.filter(smn => {
            if (smn.statusId > 0) { // 召唤物有关联状态
                ...
                this._updateStatus(pidx, [smnStatus], combatStatus, players, hidx, isExec);
            }
