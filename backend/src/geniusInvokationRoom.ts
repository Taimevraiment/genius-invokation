...
export default class GeniusInvokationRoom {
    ...
    private async _switchHero(pidx: number, hidx: number, flag: string,
        options: { socket?: Socket, diceSelect?: boolean[], isQuickAction?: boolean, notChangeTurn?: boolean } = {}) {
        await wait(() => this.taskQueue.isTaskEmpty() && !this.taskQueue.isExecuting);
        const { socket, diceSelect, notChangeTurn = false } = options;
        ...
        this.taskQueue.addTask('switchHero', [[() => { this.emit(flag, pidx, { tip: isCdt(isQuickAction, '继续{p}回合'), isActionInfo: true }) }, 1000 * (1 + +!!diceSelect)]], true);
    ...
    private _useSkill(pidx: number, skidx: number, options: { isPreview?: boolean, withCard?: Card, isSwitch?: number, isReadySkill?: boolean, otriggers?: Trigger | Trigger[] } = {}) {
        ...
        const calcAtk = (res: any, type: string, stsId: number, ahidx: number, ehidx: number, isSkill = false, isSelf = 0) => {
            ...
            const willDamages = new Array(dmgedHlen).fill(0).map((_, i) => [
            ...
            for (let i = 0; i < dmgedHlen; ++i) bWillAttach[i + (cpidx ^ 1) * (isSelf ? ehlen : ahlen)].push(willAttachs3[i]);
    ...
    private _calcDamage(
            ...
            if ( // 没有元素反应(不含冰草共存)
                ...
                if (dmgElement == ELEMENT_TYPE.Anemo && elTypes.includes(attachElement)) { // 扩散
                    ...
                    if (hasEls(ELEMENT_TYPE.Cryo, ELEMENT_TYPE.Dendro)) { // 冰草共存
                        ...
                        ++res.willDamages[getDmgIdx][0];
                        res.players[pidx].summons = this._updateSummon(pidx, [this.newSummon(115)], res.players[pidx].summons, res.players[pidx], isExec, { isSummon: isSummon[0] });
        ...
        const detectStatus = (trgs: Trigger[], hi: number, isSelf: boolean) => {
            ...
            this._doCmds(cpidx, stsres.cmds, { players: res.players, isExec });
        }
        if (!isSwirl) {
    ...
    */
            ...
            for (const slot of slots) {
                ...
                if (isExec && (tcmds.length > 0 || isAddTask)) {
                    if (!taskMark) {
                        ...
                        let intvl = 800;
                        if (tcmds.some(({ cmd }) => ['heal', 'revive'].includes(cmd))) intvl += 2000;
                        const slotHandle = () => {
                            if (!isAddTask || tcmds.length > 0) {
                                const { ndices } = this._doCmds(pidx, tcmds);
     ...
     *                isOnlyExecSts 是否只执行status的exec方法(用于calcDamage的预览)
    ...
    */
    private _detectStatus(pidx: number, otypes: StatusType | StatusType[], otrigger: Trigger | Trigger[],
        ...
        const { isExec = true, isOnlyFront = false, players = this.players, phase = this.players[pidx].phase, hasDmg = false,
            ...
            minusDiceSkill = [], isOnlyExecSts = false } = options;
        let addDiceHero = 0;
        let minusDiceHero = 0;
        let isInvalid = false;
        const cmds: Cmds[] = [];
                        ...
                        if (stsres.isAddTask) {
                            ...
                            if (!taskMark) {
                                ...
                                const statusHandle = async () => {
                                    ...
                                    const statuses = group == STATUS_GROUP.heroStatus ? aheros[hidx].heroStatus : combatStatus;
        ...
        for (let i = 0; i < cmdlen; ++i) {
            ...
            if (cmd == 'useSkill') {
                ...
                if (!isExec) {
                    ...
                    cmds[i].hidxs = [nhidx];
                    setTimeout(() => this._switchHero(cpidx, nhidx, 'doCmd--switch', { socket, notChangeTurn: true }), cnt);
                }
