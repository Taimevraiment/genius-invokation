...
import {
    ...
    MAX_SUMMON_COUNT, MAX_SUPPORT_COUNT, PLAYER_COUNT
...
export default class GeniusInvokationRoom {
    ...
    private async _switchHero(pidx: number, hidx: number, socket: Socket | undefined, flag: string, diceSelect?: boolean[]) {
        ...
        this.taskQueue.addTask('switchHero', [[() => { this.emit(flag, pidx, { tip: isCdt(isQuickAction, '继续{p}回合') }) }, 1000]], true);
    ...
    private _doDamage(pidx: number, damageVO: DamageVO, isSwitch: number = -1) {
        ...
        this.emit(damageVO.dmgSource + '-doDamage', pidx, { damageVO, actionInfo });
    }
    /**
     * 检测治疗
     * @param pidx 玩家序号
    ...
    _doStatusAtk(stsTask: StatusTask) {
        return new Promise<boolean>(async resolve => {
            ...
            this._doDamage(pidx ^ isSelf, {
                ...
                elTips,
                willHeals,
                selected: [pidx, getAtkStatus().group, getAtkStatus().group == STATUS_GROUP.heroStatus ? getObjIdxById(aheros[ahidx].heroStatus, getAtkStatus().id) : getObjIdxById(combatStatus, getAtkStatus().id)],
            });
    ...
    */
                        ...
                        const summonHandle1 = async () => {
                            ...
                            if (smnexecres?.cmds) {
                                ...
                                const { atkedIdx: [tarHidx] = [-1], willDamages = [], willHeals = [], dmgElements = [], elTips = [] }
                                    = this._doCmds(pidx, smnexecres.cmds, { heal: summon.shieldOrHeal, damages, heros: aHeros });
                                this._doDamage(pidx, {
                                    dmgSource: 'summon',
                                    atkPidx: pidx,
                                    atkHidx: -1,
    ...
    _doCmds(pidx: number, cmds?: Cmds[],
        ...
        const player = players[pidx];
        const opponent = players[pidx ^ 1];
        const { withCard, hidxs: chidxs, heal = 0, isExec = true, ahidx = player.hidx, ehidx = opponent.hidx, heroIdx = -1,
