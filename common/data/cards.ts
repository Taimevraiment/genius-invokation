...
const allCards: Record<number, () => CardBuilder> = {
    ...
    311109: () => new CardBuilder(381).name('金流监督').since('v4.7.0').weapon().costSame(2).perCnt(2)
        ...
        .handle((card, event, ver) => {
            const { heros = [], heal = [], hidxs: [hidx] = [], trigger = '' } = event;
    ...
    312021: () => new CardBuilder(307).name('灵光明烁之心').since('v4.3.0').artifact().costSame(0).perCnt(1)
        ...
        .handle((card, event) => {
            const { heros = [], hidxs: [hidx] = [], isExecTask = false } = event;
            if (card.perCnt <= 0 || (!isExecTask && !heros[hidx]?.isFront)) return;
            return {
                trigger: ['getdmg'],
                execmds: [{ cmd: 'getCard', cnt: 1 }],
                exec: () => { --card.perCnt }
    ...
    312022: () => new CardBuilder(308).name('花海甘露之光').since('v4.3.0').artifact().costSame(1).perCnt(1)
        ...
        .handle((card, event) => {
            ...
            const isGetCard = trigger == 'getdmg' && card.perCnt > 0 && (!isExecTask || heros[hidxs[0]]?.isFront);
            return {
            ...
            return {
                ...
                exec: () => {
                    if (isPhaseEnd) {
                        ...
                        card.useCnt = Math.max(0, card.useCnt - (hcard.cost + hcard.anydice - mdc));
                        ...
                        card.useCnt = Math.max(0, card.useCnt - (skillcost - minusDiceSkill.find(([eid]) => eid == card.entityId)![1]));
                    }
                }
            }
        }),
    ...
    224021: () => new CardBuilder(296).name('悲号回唱').since('v4.3.0').talent(1).talent(-1, 'v4.4.0').costElectro(3).costSame(0, 'v4.4.0').perCnt(1)
        ...
        .handle((card, event) => {
            if (card.perCnt <= 0) return;
            const { dmg = [], eheros = [], isExecTask = false } = event;
