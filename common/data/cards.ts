...
const allCards: Record<number, () => CardBuilder> = {
    ...
    312001: () => new CardBuilder(148).name('冒险家头带').offline('v2').relic().costSame(1).perCnt(3)
        ...
        .handle((card, event) => {
            const { hero, execmds } = event;
            if (card.perCnt <= 0 || !hero.isHert) return;
    ...
    332015: () => new CardBuilder(255).name('深渊的呼唤').tag(CARD_TAG.LocalResonance).costSame(2)
        ...
        .handle((_, event) => {
            ...
            return { exec: () => cmds.getSummon(randomInArr(smnIds)) }
        }),
    ...
    211101: () => new CardBuilder(338).name('以有趣相关为要义').since('v4.5.0').talent(1).costCryo(3).perCnt(1)
        ...
        .handle((card, event) => {
            ...
            if (!heros.getFront()?.isHert) return;
            execmds.heal(2);
    ...
    212061: () => new CardBuilder(74).name('镜华风姿').since('v3.6.0').talent(1).costHydro(3)
        ...
        .handle((_, event, ver) => {
            const { eheros, hero, dmgedHidx } = event;
            if ((eheros[dmgedHidx]?.hp ?? 10) <= 6 && hero.heroStatus.has(112061)) {
    ...
    212101: () => new CardBuilder(339).name('古海孑遗的权柄').since('v4.5.0').talent(0).costHydro(1).anydice(2)
        ...
        .handle((_, event) => {
            ...
            if (!skill?.isHeroSkill || !hero) return;
            return {
    ...
    213081: () => new CardBuilder(84).name('最终解释权').since('v3.8.0').talent(0).costPyro(1).anydice(2)
        ...
        .handle((_, event, ver) => {
            const { isChargedAtk, hero, eheros = [], dmgedHidx = -1, execmds } = event;
            if (!isChargedAtk) return;
            if (ver.gte('v4.2.0') && hero.heroStatus.has(113081)) execmds.getCard(1);
            return { triggers: 'skilltype1', addDmgCdt: isCdt((eheros[dmgedHidx]?.hp ?? 10) <= 6, 1) }
        }),
    ...
    226011: () => new CardBuilder(119).name('重铸：岩盔').offline('v2').talent(2).costGeo(4).energy(2)
        ...
        .handle((card, event) => {
            const { skill, hero, execmds } = event;
            if (getHidById(skill?.id ?? -1) != card.userType || !hero) return;
