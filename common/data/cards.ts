...
const allCards: Record<number, () => CardBuilder> = {
    ...
    311405: () => new CardBuilder(141).name('薙草之稻光').since('v3.7.0').weapon().costSame(3).perCnt(1)
        ...
        .handle((card, event) => {
            const { hero, slotUse, isExecTask, cmds, execmds } = event;
            if (!hero) return;
    ...
    311503: () => new CardBuilder(145).name('风鹰剑').offline('v1').weapon().costSame(3).perCnt(2)
        ...
        .handle((card, event) => {
            const { hero, isExecTask, execmds } = event;
            if (!hero) return;
    ...
    312003: () => new CardBuilder(150).name('游医的方巾').offline('v2').relic().costSame(1).perCnt(1)
        ...
        .handle((card, event) => {
            ...
            execmds.heal(1, { hidxs: allHidxs(heros) });
    ...
    312010: () => new CardBuilder(157).name('千岩牢固').since('v3.7.0').relic().costSame(3).perCnt(1)
        ...
        .handle((card, event) => {
            const { hero, trigger, isExecTask, execmds } = event;
    ...
    312016: () => new CardBuilder(163).name('海染砗磲').since('v4.2.0').relic().costAny(3).useCnt(0).isResetPerCnt()
        ...
        .handle((card, event) => {
            const { trigger = '', heal = [], sktype = SKILL_TYPE.Vehicle, cmds } = event;
            const allHeal = heal.reduce((a, b) => a + Math.max(0, b), 0);
            const isHeal = ['heal', 'other-heal'].includes(trigger);
            if (isHeal && (allHeal == 0 || card.useCnt == 2)) return;
            cmds.heal(2);
    ...
    312017: () => new CardBuilder(164).name('沙王的投影').offline('v1').since('v4.2.0').relic().costSame(1).perCnt(1)
        ...
        .handle((card, event) => {
            const { hero, hasDmg, isExecTask, cmds, execmds } = event;
            cmds.getCard(1);
            execmds.getCard(1);
            return {
    ...
    312018: () => new CardBuilder(304).name('饰金之梦').since('v4.3.0').relic().costSame(3).costAny(3, 'v4.5.0').perCnt(2)
        ...
        .handle((card, event, ver) => {
            const { heros = [], hero, hasDmg, isExecTask, cmds, execmds } = event;
    ...
    330011: () => new CardBuilder(487).name('为「死」而战').since('v5.7.0').legend().costSame(1)
        ...
        .handle((_, event) => {
            const { cmds, cmdsBefore, heros } = event;
            cmdsBefore.getCard(1);
    ...
    331202: () => new CardBuilder(226).name('元素共鸣：愈疗之水').offline('v1').subtype(CARD_SUBTYPE.ElementResonance).costHydro(1)
        ...
        .handle((_, event) => {
            ...
            cmds.heal(2, { hidxs: hero?.hidx }).heal(1, { hidxs: getBackHidxs(heros) });
            return { isValid: heros.some(h => h.hp < h.maxHp) }
        }),
    ...
    332041: () => new CardBuilder(442).name('强劲冲浪拍档！').since('v5.2.0').event().costSame(0)
        ...
        .handle((_, event) => {
            ...
            if (randomInt) {
                ...
                if (esummons.length) cmds.summonTrigger(randomInt(esummons.length - 1), true);
            }
    ...
    211091: () => new CardBuilder(288).name('归芒携信').since('v4.3.0').talent(1).costCryo(3)
        ...
        .handle((_, event) => {
            const { dmgSource = -1, execmds } = event;
    ...
    212051: () => new CardBuilder(73).name('匣中玉栉').since('v3.5.0').offline('v2').talent(2).costHydro(3).energy(2)
        ...
        .handle((_, event) => {
            const { hero, isSummon = -1 } = event;
            if (!hasObjById(hero?.heroStatus, 112052) || isSummon != 112051) return;
    ...
    213081: () => new CardBuilder(84).name('最终解释权').since('v3.8.0').talent(0).costPyro(1).anydice(2)
        ...
        .handle((_, event, ver) => {
            const { isChargedAtk, hero, eheros = [], dmgedHidx = -1, isExecTask, execmds } = event;
            if (!isChargedAtk && !isExecTask) return;
            if (ver.gte('v4.2.0') && (hasObjById(hero?.heroStatus, 113081) || isExecTask)) execmds.getCard(1);
    ...
    214121: () => new CardBuilder(447).name('破夜的明焰').since('v5.3.0').talent(1).costElectro(2)
        ...
        .handle((_, event) => {
            const { hero, sktype = SKILL_TYPE.Vehicle, execmds } = event;
    ...
    215061: () => new CardBuilder(101).name('梦迹一风').since('v4.1.0').talent(1).costAnemo(4)
        ...
        .handle((_, event) => {
            const { isChargedAtk, hero, isExecTask, execmds } = event;
            if (!hero) return;
    ...
    223031: () => new CardBuilder(295).name('魔蝎烈祸').since('v4.3.0').talent(2).costPyro(3).energy(2)
        ...
        .handle((_, event, ver) => {
            ...
            const { skid = -1, trigger, execmds } = event;
            const triggers: Trigger[] = ['phase-end'];
    ...
    224021: () => new CardBuilder(296).name('悲号回唱').since('v4.3.0').talent(1).talent(-1, 'v4.4.0').costElectro(3).costSame(0, 'v4.4.0').perCnt(1)
        ...
        .handle((card, event) => {
            if (card.perCnt <= 0) return;
            const { dmg = [], eheros = [], isExecTask, execmds } = event;
    ...
    226011: () => new CardBuilder(119).name('重铸：岩盔').offline('v2').talent(2).costGeo(4).energy(2)
        ...
        .handle((card, event) => {
            const { skid = -1, hero, execmds } = event;
    ...
    112133: () => new CardBuilder().name('激愈水球·小').event().costSame(0)
        ...
        .handle((_, event) => {
            const { heros, execmds } = event;
            execmds.heal(1, { hidxs: allHidxs(heros) }).getStatus(112101);
            return { triggers: 'getcard' }
        }),
    ...
    127032: () => new CardBuilder().name('厄灵·草之灵蛇').vehicle().costSame(0).useCnt(2).perCnt(1).isSpReset()
        ...
        .handle((card, event) => {
            const { skid = -1, combatStatus, heros, slotUse, hcard, reset, trigger, execmds } = event;
            const hid = getHidById(card.id);
