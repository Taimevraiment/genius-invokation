...
const elCard = (shareId: number, element: PureElementType) => {
    ...
    return new CardBuilder(shareId).name('元素共鸣：交织之' + elName[0])
...
const allCards: Record<number, () => CardBuilder> = {
    ...
    311402: () => new CardBuilder(138).name('千岩长枪').weapon().costSame(3)
        .description('【角色造成的伤害+1】。；【入场时：】队伍中每有一名｢璃月｣角色，此牌就为附属的角色提供1点[护盾]。(最多3点)')
    ...
    311409: () => new CardBuilder(402).name('勘探钻机').since('v4.8.0').weapon().costSame(2).tag(CARD_TAG.Barrier).useCnt(0).perCnt(2)
        .description('【所附属角色受到伤害时：】如可能，[舍弃]原本元素骰费用最高的1张手牌，以抵消1点伤害，然后累积1点｢团结｣。(每回合最多触发2次)；【角色使用技能时：】如果此牌已有｢团结｣，则消耗所有｢团结｣，使此技能伤害+1，并且每消耗1点｢团结｣就抓1张牌。')
        ...
        .handle((card, event) => {
            ...
            if (restDmg > -1) {
                ...
                ++card.useCnt;
                --card.perCnt;
                return { restDmg: restDmg - 1, execmds: [{ cmd: 'discard', mode: CMD_MODE.HighHandCard }] }
            }
    ...
    312001: () => new CardBuilder(148).name('冒险家头带').artifact().costSame(1).perCnt(3)
        ...
        .handle((card, event) => {
            ...
            if (card.perCnt <= 0 || curHero?.maxHp == curHero?.hp) return;
            return {
                trigger: ['skilltype1'],
                execmds: [{ cmd: 'heal', cnt: 1, hidxs }],
                exec: () => { --card.perCnt }
            }
        }),
    ...
    312005: () => new CardBuilder(152).name('教官的帽子').artifact().costAny(2).perCnt(3)
    ...
    312006: () => new CardBuilder(153).name('流放者头冠').artifact().costAny(2).perCnt(1)
        ...
        .handle((card, event) => {
            ...
            return {
                trigger: ['skilltype3'],
                execmds: [{ cmd: 'getEnergy', cnt: 1, hidxs: getBackHidxs(heros) }],
                exec: () => { --card.perCnt }
            }
        }),
    ...
    312009: () => new CardBuilder(156).name('将帅兜鍪').since('v3.5.0').artifact().costSame(2)
        ...
        .handle((_, event, ver) => ({
            trigger: ['phase-start'],
            execmds: [{ cmd: 'getStatus', status: [newStatus(ver)(301201)], hidxs: event.hidxs }],
        })),
    ...
    312010: () => new CardBuilder(157).name('千岩牢固').since('v3.7.0').artifact().costSame(3).perCnt(1)
        ...
        .handle((card, event, ver) => {
            ...
            const isGetDmg = trigger == 'getdmg' && card.perCnt > 0 && heros[hidxs[0]].isFront;
            return {
                trigger: ['phase-start', 'getdmg'],
                isAddTask: !isExecTask,
                execmds: isCdt<Cmds[]>(trigger == 'phase-start', [{ cmd: 'getStatus', status: [newStatus(ver)(301201)], hidxs }],
    ...
    312013: () => new CardBuilder(160).name('无常之面').since('v3.7.0').artifact().costAny(2).costSame(2, 'v4.0.0').perCnt(1)
    ...
    312014: () => new CardBuilder(161).name('追忆之注连').since('v3.7.0').artifact().costAny(3).costSame(3, 'v4.0.0').perCnt(1)
    ...
    312015: () => new CardBuilder(162).name('海祇之冠').since('v4.1.0').artifact().costSame(1).useCnt(0)
        .description('我方角色每受到3点治疗，此牌就累计1个｢海染泡沫｣。(最多累积2个)；【角色造成伤害时：】消耗所有｢海染泡沫｣，每消耗1个都能使造成的伤害+1。')
    ...
    312016: () => new CardBuilder(163).name('海染砗磲').since('v4.2.0').artifact().costAny(3).useCnt(0.681).useCnt(1.021, 'v4.3.0')
        .description('【入场时：】治疗所附属角色2点。；我方角色每受到3点治疗，此牌就累计1个｢海染泡沫｣。(最多累积2个)；【角色造成伤害时：】消耗所有｢海染泡沫｣，每消耗1个都能使造成的伤害+1。')
    ...
    312017: () => new CardBuilder(164).name('沙王的投影').since('v4.2.0').artifact().costSame(1).perCnt(1)
    ...
    312021: () => new CardBuilder(307).name('灵光明烁之心').since('v4.3.0').artifact().costSame(0).perCnt(1)
        ...
        .handle((card, event) => {
            const { heros = [], hidxs: [hidx] = [] } = event;
            if (card.perCnt <= 0 || !heros[hidx]?.isFront) return;
            return {
                trigger: ['getdmg'],
                execmds: [{ cmd: 'getCard', cnt: 1 }],
                exec: () => { --card.perCnt }
    ...
    312026: () => new CardBuilder(386).name('黄金剧团').since('v4.7.0').artifact().costSame(2).useCnt(0)
        ...
        .handle((card, event) => {
            ...
            return {
                ...
                exec: () => {
                    if (isPhaseEnd) {
                        card.useCnt = Math.max(4, card.useCnt + 2);
                    } else if (trigger == 'card' && isMinusDiceTalent && hcard) {
                        card.useCnt -= hcard.cost + hcard.anydice - mdc;
                    } else if (trigger == 'skilltype2' && isMinusDiceSkill) {
                        const skill = heros[hidx]?.skills.find(sk => sk.id == skid)?.cost ?? [{ cnt: 0 }, { cnt: 0 }];
    ...
    323004: () => new CardBuilder(217).name('寻宝仙灵').since('v3.7.0').item().costSame(1)
    ...
    330001: () => new CardBuilder(218).name('旧时庭园').since('v3.8.0').legend().costSame(0)
        ...
        .handle((_, event, ver) => {
            ...
            return { status: [newStatus(ver)(300001)], isValid }
        }),
    ...
    330006: () => new CardBuilder(314).name('裁定之时').since('v4.3.0').legend().costSame(1)
        ...
        .handle((_c, _e, ver) => ({ statusOppo: [newStatus(ver)(300003)] })),
    ...
    330007: () => new CardBuilder(346).name('抗争之日·碎梦之时').since('v4.5.0').legend().costSame(0).canSelectHero(1)
        ...
        .handle((_c, _e, ver) => ({ status: [newStatus(ver)(300004)] })),
    ...
    331102: () => new CardBuilder(224).name('元素共鸣：粉碎之冰').subtype(CARD_SUBTYPE.ElementResonance).costCryo(1)
        ...
        .handle((_, { hidxs }, ver) => ({ status: [newStatus(ver)(303112)], hidxs })),
    ...
    331202: () => new CardBuilder(226).name('元素共鸣：愈疗之水').subtype(CARD_SUBTYPE.ElementResonance).costHydro(1)
    ...
    331302: () => new CardBuilder(228).name('元素共鸣：热诚之火').subtype(CARD_SUBTYPE.ElementResonance).costPyro(1)
        ...
        .handle((_, { hidxs }, ver) => ({ status: [newStatus(ver)(303132)], hidxs })),
    ...
    331402: () => new CardBuilder(230).name('元素共鸣：强能之雷').subtype(CARD_SUBTYPE.ElementResonance).costElectro(1)
        .description('我方一名充能未满的角色获得1点[充能]。(出战角色优先)')
        ...
        .handle((_, event) => {
            ...
            for (let i = 0; i < heros.length; ++i) {
                const chidx = (fhidx + i) % heros.length;
                const chero = heros[chidx];
                if (chero.hp > 0 && chero.energy < chero.maxEnergy) {
                    hidxs.push(chidx);
                    break;
                }
            }
    ...
    331502: () => new CardBuilder(232).name('元素共鸣：迅捷之风').subtype(CARD_SUBTYPE.ElementResonance).costAnemo(1)
        ...
        .handle((_, event) => {
            const { heros = [], hidxs } = event;
            return {
                cmds: [{ cmd: 'switch-to', hidxs }, { cmd: 'getDice', cnt: 1, element: DICE_COST_TYPE.Omni }],
    ...
    331602: () => new CardBuilder(234).name('元素共鸣：坚定之岩').subtype(CARD_SUBTYPE.ElementResonance).costGeo(1)
        ...
        .handle((_c, _e, ver) => ({ status: [newStatus(ver)(303162)] })),
    ...
    331702: () => new CardBuilder(236).name('元素共鸣：蔓生之草').subtype(CARD_SUBTYPE.ElementResonance).costDendro(1)
        ...
        .handle((_, event, ver) => {
            ...
            return {
                status: [newStatus(ver)(303172)],
                exec: () => {
                    combatStatus.forEach(ost => (ost.id == 116 || ost.id == 117) && ++ost.useCnt);
                    summons.forEach(smn => smn.id == 115 && ++smn.useCnt);
                }
            }
        }),
    ...
    331801: () => new CardBuilder(237).name('风与自由').since('v3.7.0').tag(CARD_TAG.LocalResonance).costSame(0).costSame(1, 'v4.3.0')
        ...
        .handle((_, event, ver) => {
            const { heros = [] } = event;
            return { isValid: ver < 'v4.1.0' || allHidxs(heros).length > 1, status: [newStatus(ver)(303181)] }
        }),
    ...
    331802: () => new CardBuilder(238).name('岩与契约').since('v3.7.0').tag(CARD_TAG.LocalResonance).costAny(3)
        ...
        .handle((_c, _e, ver) => ({ status: [newStatus(ver)(303182)] })),
    ...
    332002: () => new CardBuilder(242).name('换班时间').event().costSame(0)
        ...
        .handle((_, event, ver) => {
            const { heros = [] } = event;
            return { isValid: allHidxs(heros).length > 1, status: [newStatus(ver)(303202)] }
        }),

    332003: () => new CardBuilder(243).name('一掷乾坤').event().costSame(0)
    ...
    332004: () => new CardBuilder(244).name('运筹帷幄').event().costSame(1)
        .description('抓2张牌。')
    ...
    332005: () => new CardBuilder(245).name('本大爷还没有输！').event().costSame(0)
        ...
        .handle((_, event, ver) => {
            ...
            return {
                ...
                status: isCdt(ver >= '4.0.0', [newStatus(ver)(303205)]),
            }
        }),

    332006: () => new CardBuilder(246).name('交给我吧！').event().costSame(0)
        ...
        .handle((_, event, ver) => {
            const { heros = [] } = event;
            return { isValid: allHidxs(heros).length > 1, status: [newStatus(ver)(303206)] }
        }),

    332007: () => new CardBuilder(247).name('鹤归之时').event().costSame(1)
        ...
        .handle((_, event, ver) => {
            const { heros = [] } = event;
            return { isValid: allHidxs(heros).length > 1, status: [newStatus(ver)(303207)] }
        }),

    332008: () => new CardBuilder(248).name('星天之兆').event().costAny(2)
    ...
    332009: () => new CardBuilder(249).name('白垩之术').event().costSame(0)
    ...
    332011: () => new CardBuilder(251).name('神宝迁宫祝词').event().costSame(0).canSelectHero(2)
    ...
    332012: () => new CardBuilder(252).name('快快缝补术').event().costSame(1).canSelectSummon(1)
        ...
        .handle((_, event) => ({
            exec: () => {
                const { summons = [], hidxs: [smnIdx] = [] } = event;
                const selectSmn = summons[smnIdx];
        ...
        .handle((_, event, ver) => ({
            exec: () => {
                ...
                if (ver < 'v3.7.0') {
                    ...
                    selectSmn.useCnt = Math.max(0, selectSmn.useCnt - 2);
                }
            }
        })),
    ...
    211041: () => new CardBuilder(64).name('吐纳真定').talent(1).costCryo(3).costCryo(4, 'v4.2.0')
    ...
    211051: () => new CardBuilder(65).name('寒天宣命祝词').talent().costCryo(2).perCnt(1)
    ...
    212031: () => new CardBuilder(71).name('沉没的预言').talent(2).costHydro(3).energy(3)
        ...
        .handle((_, event) => {
            const { heros = [], hidxs: [hidx] = [] } = event;
            return { trigger: ['elReaction-Hydro'], addDmgCdt: isCdt(heros[hidx]?.isFront, 2) }
        }),
    ...
    302214: () => new CardBuilder().name('托皮娅的声援').event().costSame(0)
        ...
        .src('https://act-upload.mihoyo.com/wiki-user-upload/2024/07/07/258999284/b8cc5a1a4f585ab31d7af7621fe7cc9a_7205746796255007122.png')
        .handle((_c, _e, ver) => ({ cmds: [{ cmd: 'getCard', cnt: 2 }], status: [newStatus(ver)(302216)], statusOppo: [newStatus(ver)(302216)] })),
