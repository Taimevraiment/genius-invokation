...
import { CARD_SUBTYPE, DICE_COST_TYPE, DiceCostType, ELEMENT_TYPE, PureElementType, VERSION, Version } from '../constant/enum.js';
...
const senlin1Weapon = (shareId: number, name: string, stsId: number) => {
    return new CardBuilder(shareId).name(name)
        .description('【角色造成的伤害+1】。；【入场时：】所附属角色在本回合中，下次对角色打出｢天赋｣或使用｢元素战技｣时少花费2个元素骰。')
        .weapon().costSame(3).handle((_c, _e, ver) => ({ addDmg: 1, status: [newStatus(ver)(stsId, name)] }));
}
...
const allCards: Record<number, () => CardBuilder> = {
    ...
    // 69: () => new GICard(69, '勘探钻机', '【所附属角色受到伤害时：】如可能，[舍弃]原本元素骰费用最高的1张手牌，以抵消1点伤害，然后累积1点｢团结｣。(每回合最多触发2次)；【角色使用技能时：】如果此牌已有｢团结｣，则消耗所有｢团结｣，使此技能伤害+1，并且每消耗1点｢团结｣就抓1张牌。',
    ...
    // 117: () => new GICard(117, '沙王的投影', '【入场时：】抓1张牌。；【所附属角色为出战角色期间，敌方受到元素反应伤害时：】抓1张牌。(每回合1次)',
    ...
    // 118: () => new GICard(118, '饰金之梦', '【入场时：】生成1个所附属角色类型的元素骰。如果我方队伍中存在3种不同元素类型的角色，则改为生成2个。；【所附属角色为出战角色期间，敌方受到元素反应伤害时：】抓1张牌。(每回合1次)',
    ...
    // 119: () => new GICard(119, '浮溯之珏', '【角色使用｢普通攻击｣后：】抓1张牌。(每回合1次)',
    ...
    // 120: () => new GICard(120, '来歆余响', '【角色使用｢普通攻击｣后：】抓1张牌。(每回合1次)；【角色使用技能后：】如果我方元素骰数量不多于手牌数量，则生成1个所附属角色类型的元素骰。(每回合1次)',
    ...
    // 121: () => new GICard(121, 307, '灵光明烁之心', '【角色受到伤害后：】如果所附属角色为｢出战角色｣，则抓1张牌。(每回合1次)',
    ...
    // 122: () => new GICard(122, '花海甘露之光', '【角色受到伤害后：】如果所附属角色为｢出战角色｣，则抓1张牌，并且在本回合结束阶段中治疗所附属角色1点。(每回合1次)',
    ...
    // 123: () => new GICard(123, '老兵的容颜', '【角色受到伤害或治疗后：】根据本回合触发此效果的次数，执行不同的效果。；【第一次触发：】生成1个此角色类型的元素骰。；【第二次触发：】抓1张牌。',
    ...
    // 124: () => new GICard(124, '逐影猎人', '【角色受到伤害或治疗后：】根据本回合触发此效果的次数，执行不同的效果。；【第一次触发：】生成1个此角色类型的元素骰。；【第二次触发：】抓1张牌。；【第四次触发：】生成1个此角色类型的元素骰。',
    ...
    // 404: () => new GICard(404, '寻宝仙灵', '【我方角色使用技能后：】此牌累积1个｢寻宝线索｣。；当此牌已累积3个｢寻宝线索｣时，弃置此牌并抓3张牌。',
    ...
    // 407: () => new GICard(407, '流明石触媒', '【我方打出行动牌后：】如果此牌在场期间本回合中我方已打出3张行动牌，则抓1张牌并生成1个[万能元素骰]。(每回合1次)；[可用次数]：3',
    ...
    // 565: () => new GICard(565, '万家灶火', '【第1回合打出此牌时：】如果我方牌组中初始包含至少2张不同的｢天赋｣牌，则抓1张｢天赋｣牌。；【第2回合及以后打出此牌时：】我方抓【当前回合数-1】数量的牌。(最多抓4张)',
    ...
    // 572: () => new GICard(572, '岩与契约', '【下回合行动阶段开始时：】生成3点[万能元素骰]，并抓1张牌。',
    ...
    // 574: () => new GICard(574, '草与智慧', '抓1张牌。然后，选择任意手牌替换。',
    ...
    331101: () => elCard(225, ELEMENT_TYPE.Cryo)
    ...
    331201: () => elCard(227, ELEMENT_TYPE.Hydro)
    ...
    331301: () => elCard(229, ELEMENT_TYPE.Pyro)
    ...
    331401: () => elCard(231, ELEMENT_TYPE.Electro)
    ...
    331501: () => elCard(233, ELEMENT_TYPE.Anemo)
    ...
    331601: () => elCard(235, ELEMENT_TYPE.Geo)
    ...
    331701: () => elCard(237, ELEMENT_TYPE.Dendro)
    ...
    211011: () => new CardBuilder(61).name('唯此一心')
        ...
        .talent(2).costCryo(3).handle((card, event, ver) => {
    ...
    211021: () => new CardBuilder(62).name('猫爪冰摇')
        ...
        .talent(1).costCryo(3).costCryo(4, 'v4.1.0'),

    211031: () => new CardBuilder(63).name('冷血之剑')
        ...
        .talent(1).costCryo(4).perCnt(1).handle((card, event) => {
            if (card.perCnt <= 0) return;
            const { hidxs } = event;
            return {
    ...
    211041: () => new CardBuilder(64).name('吐纳真定')
        ...
        .talent(1).costCryo(3).costCryo(4, 'v4.2.0'),

    211051: () => new CardBuilder(65).name('寒天宣命祝词')
        ...
        .talent().costCryo(2).perCnt(1).handle((card, event) => ({
            trigger: ['change-to'],
    ...
    211061: () => new CardBuilder(66).name('战欲涌现').since('v3.5.0')
        ...
        .talent(2).costCryo(3).energy(2),

    211071: () => new CardBuilder(67).name('忘玄').since('v3.7.0')
    ...
    211081: () => new CardBuilder(68).name('起死回骸').since('v4.0.0')
        ...
        .talent(2).costCryo(4).costCryo(5, 'v4.7.0').energy(3).perCnt(2).isResetPerCnt(),

    211091: () => new CardBuilder(288).name('归芒携信').since('v4.3.0')
    ...
    211101: () => new CardBuilder(338).name('以有趣相关为要义').since('v4.5.0')
        ...
        .talent(1).costCryo(3).perCnt(1).handle((card, event) => {
            const { eheros = [] } = event;
    ...
    211111: () => new CardBuilder(372).name('予行恶者以惩惧').since('v4.7.0')
        ...
        .talent(0).costCryo(1).anydice(2).useCnt(0).handle((card, event) => {
    ...
    212011: () => new CardBuilder(69).name('光辉的季节')
        ...
        .talent(1).costHydro(3).costHydro(4, 'v4.2.0').perCnt(1).handle((card, event) => {
    ...
    212021: () => new CardBuilder(70).name('重帘留香')
        ...
        .talent(1).costHydro(3).costHydro(4, 'v4.2.0'),

    212031: () => new CardBuilder(71).name('沉没的预言')
        ...
        .talent(2).costHydro(3).energy(3).handle((_, event) => {
            const { heros = [], hidxs: [hidx] = [] } = event;
