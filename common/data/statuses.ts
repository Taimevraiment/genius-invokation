...
const readySkillStatus = (name: string, skill: number, shieldStatusId?: number) => {
    return new StatusBuilder(name).heroStatus().icon('buff3').useCnt(1).type(STATUS_TYPE.Sign, STATUS_TYPE.ReadySkill)
        ...
        .handle((status, event) => ({
            trigger: ['switch-from', 'useReadySkill'],
            skill,
            exec: () => {
                status.minusUseCnt();
                if (shieldStatusId) {
...
const nightSoul = (cnt: number = 0) => new StatusBuilder('夜魂加持').heroStatus().useCnt(cnt).maxCnt(2)
    ...
    .handle((status, event) => {
        ...
        return { trigger: ['get-status'], notLog: true, exec: () => { status.minusUseCnt() } }
    });
...
const statusTotal: Record<number, (...args: any) => StatusBuilder> = {
    ...
    112041: () => new StatusBuilder('远程状态').heroStatus().icon('ski,3').type(STATUS_TYPE.Usage, STATUS_TYPE.Sign)
        ...
        .handle((_, event) => ({
            trigger: ['skilltype1'],
            cmds: isCdt(event.isChargedAtk, [{ cmd: 'getStatus', status: 112043, isOppo: true }]),
        })),
            ...
            return {
                ...
                exec: (eStatus, execEvent = {}) => {
                    ...
                    if (getObjById(hs, hid)?.isFront) {
                        return { cmds: [{ cmd: 'getStatus', status: 112102 }] }
                    }
                },
            }
        }),
    ...
    112145: () => new StatusBuilder('消耗｢夜魂值｣').heroStatus().useCnt(1).type(STATUS_TYPE.Hide, STATUS_TYPE.Usage)
        .handle(status => ({ trigger: ['enter'], exec: () => status.dispose() })),
    ...
    113082: () => new StatusBuilder('灼灼').heroStatus().icon('ski,2').roundCnt(2).perCnt(1).type(STATUS_TYPE.Round, STATUS_TYPE.Usage)
        ...
        .handle((status, event) => {
            const { isChargedAtk, isMinusDiceSkill, trigger } = event;
            return {
    ...
    113112: (isTalent: boolean = false) => new StatusBuilder('炽火大铠').combatStatus().icon('ski,2').type(STATUS_TYPE.Attack)
        ...
        .handle(() => ({
            ...
            exec: eStatus => {
                eStatus?.minusUseCnt();
                return { cmds: [{ cmd: 'getStatus', status: 113111 }] }
            },
        })),
    ...
    113141: () => new StatusBuilder('血债勒令').combatStatus().useCnt(5).type(STATUS_TYPE.Usage).icon('#')
        ...
        .handle((status, event) => {
            ...
            return { trigger: ['getdmg'], cmds, exec: () => { status.minusUseCnt(hidxs.length) } }
        }),
    ...
    115051: (swirlEl: PureElementType = ELEMENT_TYPE.Anemo) =>
        new StatusBuilder('乱岚拨止' + `${swirlEl != ELEMENT_TYPE.Anemo ? `·${ELEMENT_NAME[swirlEl][0]}` : ''}`)
            ...
            .handle((status, event, ver) => {
                const { isFallAtk, isQuickAction: iqa, trigger } = event;
                if (ver.lt('v4.8.0')) {
                    return {
    ...
    115081: () => new StatusBuilder('攻袭余威').heroStatus().icon('debuff').useCnt(1).roundCnt(1)
    ...
    115091: () => new StatusBuilder('疾风示现').heroStatus().icon('buff').useCnt(1).type(STATUS_TYPE.Usage, STATUS_TYPE.ConditionalEnchant)
        ...
        .handle((status, event) => {
            ...
            return {
                ...
                exec: () => {
                    status.minusUseCnt();
                    return { cmds: [{ cmd: 'getStatus', status: 115092, isOppo: true }] }
                }
            }
        }),
    ...
    117052: () => new StatusBuilder('脉摄宣明').combatStatus().icon('ski,2').useCnt(2).type(STATUS_TYPE.Usage)
        ...
        .handle(() => ({
            ...
            exec: eStatus => {
                eStatus?.minusUseCnt();
                return { cmds: [{ cmd: 'getStatus', status: 117053 }] }
            },
        })),
    ...
    117071: () => new StatusBuilder('猫箱急件').combatStatus().icon('ski,1').useCnt(1).maxCnt(2).type(STATUS_TYPE.Attack)
        ...
        .handle((status, event) => {
            const { heros = [], hidx = -1, isExecTask } = event;
    ...
    117081: () => new StatusBuilder('梅赫拉克的助力').heroStatus().icon('ski,2').roundCnt(2)
        ...
        .handle(() => ({
            ...
            attachEl: ELEMENT_TYPE.Dendro,
            addDmgType1: 1,
            exec: () => ({ cmds: [{ cmd: 'getStatus', status: 117082 }] })
        })),
    ...
    122031: () => new StatusBuilder('水之新生').heroStatus().icon('heal2').useCnt(1).type(STATUS_TYPE.Sign, STATUS_TYPE.NonDefeat)
        ...
        .handle((_, event) => {
            ...
            return {
                ...
                exec: eStatus => {
                    eStatus?.minusUseCnt();
                    return { cmds: [{ cmd: 'getStatus', status: 122037, hidxs: [hidx] }] }
                }
            }
        }),
            ...
            return {
                ...
                exec: () => {
                    ...
                    if (drawElCode == curElCode) {
                        if (isSkill) return { cmds: [{ cmd: 'getStatus', status: 126022, hidxs: [hidx] }] }
                        return;
                    }
    ...
    127033: () => new StatusBuilder('灵蛇祝福').combatStatus().icon('ski,2').useCnt(1).maxCnt(MAX_USE_COUNT).type(STATUS_TYPE.AddDamage)
        ...
        .handle((status, event) => {
            ...
            return { trigger: ['vehicle'], addDmgCdt: 1, exec: () => { status.minusUseCnt() } }
        }),
    ...
    303226: () => new StatusBuilder('坍陷与契机（生效中）').combatStatus().icon('debuff').roundCnt(1)
        ...
        .handle(() => ({ trigger: ['active-switch-from'], addDiceHero: 1 })),
