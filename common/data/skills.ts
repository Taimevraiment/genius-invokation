...
import { skill, SkillBuilder } from "./builder/skillBuilder.js"

const ski1507x = (swirlEl: PureElementType) => {
    return skill('风风轮舞踢').description('{dealDmg}。').elemental().readySkill().damage(2).dmgElement(swirlEl);
}

export const allSkills: Record<number, () => SkillBuilder> = {
    11163: () => skill('极恶技·灭').description('消耗所有*[蛇之狡谋]，造成等同于消耗*[蛇之狡谋]数量的[冰元素伤害]，对后台角色造成2点[穿透伤害]，如果消耗了7点*[蛇之狡谋]，则改为对后台角色造成3点[穿透伤害]。')
    ...
    12074: () => skill('苍鹭震击').description('{dealDmg}。').elemental().readySkill().damage(3),

    12104: () => skill('衡平推裁').description('{dealDmg}，如果生命值至少为6，则对自身造成1点[穿透伤害]，使伤害+1。')
        .normal().readySkill().damage(2).handle(event => isCdt(event.hero.hp >= 6, { addDmgCdt: 1, pdmgSelf: 1 })),

    12112: () => skill('孤心沙龙').description('【hro】当前处于「始基力:荒性」形态，召唤【smn112111】。；（【hro】处于「始基力:芒性」形态时，会改为召唤【smn112112】。）')
    ...
    12122: () => skill('孤心沙龙').description('【hro1211】当前处于「始基力:芒性」形态，召唤【smn112112】。；（【hro1211】处于「始基力:荒性」形态时，会改为召唤【smn112111】。）')
    ...
    12135: () => skill('满满心意药剂冲击').description('{dealDmg}。').burst().readySkill().damage(2),

    13095: () => skill('焚落踢').description('{dealDmg}。').burst().readySkill().damage(3),

    13155: () => skill('驰轮车·疾驰').description('【行动阶段开始时：】生成2个[万能元素骰]。')
    ...
    15135: () => skill('勠心拳·蓄力').description('{dealDmg}。').elemental().readySkill().damage(4)
        .handle(event => {
    ...
    24015: () => skill('猜拳三连击·剪刀').description('{dealDmg}，然后[准备技能]：【rsk24016】。')
        .elemental().readySkill().damage(2).handle(() => ({ status: 124012 })),

    24016: () => skill('猜拳三连击·布').description('{dealDmg}。').elemental().readySkill().damage(3),

    24044: () => skill('霆电迸发').description('{dealDmg}。').burst().readySkill().damage(2),

    25025: () => skill('长延涤流').description('对下一个敌方后台角色{dealDmg}，然后[准备技能]：【rsk25026】。（敌方没有后台角色时，改为对出战角色造成伤害）')
    ...
    26047: () => skill('大师之击').description('{dealDmg}。').burst().readySkill().damage(3),

    27055: () => skill('催萌腐草').description('{dealDmg}。').burst().readySkill().damage(2),

    66013: () => skill('霜刺破袭').description('{dealDmg}，此角色附属【sts126022】。')
    ...
    66023: () => skill('洪流重斥').description('{dealDmg}，此角色附属【sts126022】。')
    ...
    66033: () => skill('炽焰重斥').description('{dealDmg}，此角色附属【sts126022】。')
    ...
    66043: () => skill('霆雷破袭').description('{dealDmg}，此角色附属【sts126022】。')
    ...
    1121422: () => skill('鲨鲨冲浪板').description('切换到上一个我方角色，使敌方出战角色附属1层【sts112143】。（若我方后台角色均被击倒，则额外消耗1点「夜魂值」）')
    ...
    1131541: () => skill('跃升').description('消耗1点「夜魂值」，{dealDmg}。')
    ...
    1131561: () => skill('疾驰').description('消耗1点「夜魂值」，然后[准备技能]：【rsk13155】。')
    ...
    1151021: () => skill('仙力助推').description('治疗所附属角色2点，并使其下次「普通攻击」视为[下落攻击]，伤害+1，并且技能结算后造成1点[风元素伤害]。')
    ...
    1161021: () => skill('转转冲击').description('附属角色消耗1点「夜魂值」，{dealDmg}，对敌方下一个后台角色造成1点[穿透伤害]。')
        .src('#')
    ...
    1161121: () => skill('高速腾跃').description('附属角色消耗1点「夜魂值」，抓3张牌。')
    ...
    1220511: () => skill('水泡战法').description('（需准备1个行动轮）；造成1点[水元素伤害]，敌方出战角色附属【sts122052】。')
        .src('#')
        .vehicle().costSame(1).handle(() => ({ status: 122053 })),

    1220512: () => skill('水泡封锁').description('{dealDmg}，敌方出战角色附属【sts122052】。')
        .vehicle().readySkill().damage(1).handle(() => ({ statusOppo: 122052 })),

    1230311: () => skill('炙烧攻势').description('{dealDmg}。')
    ...
    1270321: () => skill('藤蔓锋鳞').description('{dealDmg}。')
    ...
    3130011: () => skill('原海水刃').description('{dealDmg}。')
    ...
    3130021: () => skill('钩物巧技').description('{dealDmg}，窃取1张[当前元素骰费用]最高的对方手牌，然后对手抓1张牌。；如果我方手牌数不多于2，此特技少花费1个元素骰。')
    ...
    3130031: () => skill('游隙灵道').description('选择一个我方「召唤物」，立刻触发其「结束阶段」效果。（每回合最多使用1次）')
    ...
    3130041: () => skill('掘进突击').description('抓2张牌。然后，如果手牌中存在名称不存在于本局最初牌组中的牌，则提供2点[护盾]保护所附属角色。')
    ...
    3130051: () => skill('灵性援护').description('从「场地」「道具」「料理」中[挑选]1张加入手牌，并且治疗附属角色1点。')
    ...
    3130081: () => skill('昂扬状态').description('附属角色[准备技能]2次「普通攻击」。')
    ...
    3130092: () => skill('呀！呀！').description('从牌库中抓1张【特技牌】，下次我方打出【特技牌】少花费2个元素骰。')
